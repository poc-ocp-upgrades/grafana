	...
	Convey("Testing loading settings from ini file", t, func() {
		...
		Convey("Should be able to override via command line", func() {
			if runtime.GOOS == "windows" {
				cfg := NewCfg()
				cfg.Load(&CommandLineArgs{HomePath: "../../", Args: []string{`cfg:paths.data=c:\tmp\data`, `cfg:paths.logs=c:\tmp\logs`}})
				So(cfg.DataPath, ShouldEqual, `c:\tmp\data`)
				So(cfg.LogsPath, ShouldEqual, `c:\tmp\logs`)
			} else {
				cfg := NewCfg()
				cfg.Load(&CommandLineArgs{HomePath: "../../", Args: []string{"cfg:paths.data=/tmp/data", "cfg:paths.logs=/tmp/logs"}})
		...
		Convey("Should be able to override defaults via command line", func() {
			cfg := NewCfg()
			cfg.Load(&CommandLineArgs{HomePath: "../../", Args: []string{"cfg:default.server.domain=test2"}, Config: filepath.Join(HomePath, "pkg/setting/testdata/override.ini")})
			So(Domain, ShouldEqual, "test2")
		})
		Convey("Defaults can be overridden in specified config file", func() {
			if runtime.GOOS == "windows" {
				cfg := NewCfg()
				cfg.Load(&CommandLineArgs{HomePath: "../../", Config: filepath.Join(HomePath, "pkg/setting/testdata/override_windows.ini"), Args: []string{`cfg:default.paths.data=c:\tmp\data`}})
				So(cfg.DataPath, ShouldEqual, `c:\tmp\override`)
			} else {
				cfg := NewCfg()
				cfg.Load(&CommandLineArgs{HomePath: "../../", Config: filepath.Join(HomePath, "pkg/setting/testdata/override.ini"), Args: []string{"cfg:default.paths.data=/tmp/data"}})
		...
		Convey("Command line overrides specified config file", func() {
			if runtime.GOOS == "windows" {
				cfg := NewCfg()
				cfg.Load(&CommandLineArgs{HomePath: "../../", Config: filepath.Join(HomePath, "pkg/setting/testdata/override_windows.ini"), Args: []string{`cfg:paths.data=c:\tmp\data`}})
				So(cfg.DataPath, ShouldEqual, `c:\tmp\data`)
			} else {
				cfg := NewCfg()
				cfg.Load(&CommandLineArgs{HomePath: "../../", Config: filepath.Join(HomePath, "pkg/setting/testdata/override.ini"), Args: []string{"cfg:paths.data=/tmp/data"}})
				So(cfg.DataPath, ShouldEqual, "/tmp/data")
			}
		})
		Convey("Can use environment variables in config values", func() {
			if runtime.GOOS == "windows" {
				...
				cfg := NewCfg()
				cfg.Load(&CommandLineArgs{HomePath: "../../", Args: []string{"cfg:paths.data=${GF_DATA_PATH}"}})
		...
		Convey("Reading callback_url should add trailing slash", func() {
			cfg := NewCfg()
			cfg.Load(&CommandLineArgs{HomePath: "../../", Args: []string{"cfg:rendering.callback_url=http://myserver/renderer"}})
