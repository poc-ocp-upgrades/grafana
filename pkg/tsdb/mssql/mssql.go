...
func newMssqlQueryEndpoint(datasource *models.DataSource) (tsdb.TsdbQueryEndpoint, error) {
	...
	config := tsdb.SqlQueryEndpointConfiguration{DriverName: "mssql", ConnectionString: cnnstr, Datasource: datasource, MetricColumnTypes: []string{"VARCHAR", "CHAR", "NVARCHAR", "NCHAR"}}
	rowTransformer := mssqlRowTransformer{log: logger}
...
func generateConnectionString(datasource *models.DataSource) string {
	...
	connStr := fmt.Sprintf("server=%s;port=%s;database=%s;user id=%s;password=%s;", server, port, datasource.Database, datasource.User, password)
	if encrypt != "false" {
...
type mssqlRowTransformer struct{ log log.Logger }
