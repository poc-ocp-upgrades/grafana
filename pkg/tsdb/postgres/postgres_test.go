...
func TestPostgres(t *testing.T) {
	...
	Convey("PostgreSQL", t, func() {
		...
		endpoint, err := newPostgresQueryEndpoint(&models.DataSource{JsonData: simplejson.New(), SecureJsonData: securejsondata.SecureJsonData{}})
		So(err, ShouldBeNil)
		sess := x.NewSession()
		...
		Convey("Given a table with different native data types", func() {
			...
			Convey("When doing a table query should map Postgres column types to Go types", func() {
				query := &tsdb.TsdbQuery{Queries: []*tsdb.Query{{Model: simplejson.NewFromAny(map[string]interface{}{"rawSql": "SELECT * FROM postgres_types", "format": "table"}), RefId: "A"}}}
		...
		Convey("Given a table with metrics that lacks data for some series ", func() {
			...
			Convey("When doing a metric query using timeGroup", func() {
				query := &tsdb.TsdbQuery{Queries: []*tsdb.Query{{Model: simplejson.NewFromAny(map[string]interface{}{"rawSql": "SELECT $__timeGroup(time, '5m') AS time, avg(value) as value FROM metric GROUP BY 1 ORDER BY 1", "format": "time_series"}), RefId: "A"}}}
				resp, err := endpoint.Query(context.Background(), nil, query)
				So(err, ShouldBeNil)
				queryResult := resp.Results["A"]
				So(queryResult.Error, ShouldBeNil)
		...
		Convey("Given a table with metrics having multiple values and measurements", func() {
			...
			Convey("When doing a metric query with metric column and multiple value columns", func() {
				query := &tsdb.TsdbQuery{Queries: []*tsdb.Query{{Model: simplejson.NewFromAny(map[string]interface{}{"rawSql": `SELECT $__timeEpoch(time), measurement as metric, "valueOne", "valueTwo" FROM metric_values ORDER BY 1`, "format": "time_series"}), RefId: "A"}}}
				resp, err := endpoint.Query(context.Background(), nil, query)
				So(err, ShouldBeNil)
				queryResult := resp.Results["A"]
				So(queryResult.Error, ShouldBeNil)
