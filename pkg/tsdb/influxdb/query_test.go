...
func TestInfluxdbQueryBuilder(t *testing.T) {
	...
	Convey("Influxdb query builder", t, func() {
		...
		Convey("can build simple query", func() {
			query := &Query{Selects: []*Select{{*qp1, *qp2}}, Measurement: "cpu", Policy: "policy", GroupBy: []*QueryPart{groupBy1, groupBy3}, Interval: time.Second * 10}
		...
		Convey("can build query with group bys", func() {
			query := &Query{Selects: []*Select{{*qp1, *qp2}}, Measurement: "cpu", GroupBy: []*QueryPart{groupBy1, groupBy2, groupBy3}, Tags: []*Tag{tag1, tag2}, Interval: time.Second * 5}
			rawQuery, err := query.Build(queryContext)
			So(err, ShouldBeNil)
		...
		Convey("can build query with math part", func() {
			query := &Query{Selects: []*Select{{*qp1, *qp2, *mathPartDivideBy100}}, Measurement: "cpu", Interval: time.Second * 5}
			rawQuery, err := query.Build(queryContext)
			So(err, ShouldBeNil)
		...
		Convey("can build query with math part using $__interval_ms variable", func() {
			query := &Query{Selects: []*Select{{*qp1, *qp2, *mathPartDivideByIntervalMs}}, Measurement: "cpu", Interval: time.Second * 5}
			rawQuery, err := query.Build(queryContext)
			So(err, ShouldBeNil)
		...
		Convey("can build query with old $interval variable", func() {
			query := &Query{Selects: []*Select{{*qp1, *qp2}}, Measurement: "cpu", Policy: "", GroupBy: []*QueryPart{groupByOldInterval}}
			rawQuery, err := query.Build(queryContext)
			So(err, ShouldBeNil)
		...
		Convey("can build query from raw query", func() {
			query := &Query{Selects: []*Select{{*qp1, *qp2}}, Measurement: "cpu", Policy: "policy", GroupBy: []*QueryPart{groupBy1, groupBy3}, Interval: time.Second * 10, RawQuery: "Raw query", UseRawQuery: true}
			rawQuery, err := query.Build(queryContext)
			So(err, ShouldBeNil)
