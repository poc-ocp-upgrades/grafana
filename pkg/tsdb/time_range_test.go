...
func TestTimeRange(t *testing.T) {
	...
	Convey("Time range", t, func() {
		...
		Convey("Can parse 5m, now", func() {
			tr := TimeRange{From: "5m", To: "now", now: now}
			Convey("5m ago ", func() {
		...
		Convey("Can parse 5h, now-10m", func() {
			tr := TimeRange{From: "5h", To: "now-10m", now: now}
			Convey("5h ago ", func() {
		...
		Convey("can parse unix epocs", func() {
			var err error
			tr := TimeRange{From: "1474973725473", To: "1474975757930", now: now}
			res, err := tr.ParseFrom()
			So(err, ShouldBeNil)
		...
		Convey("Cannot parse asdf", func() {
			var err error
			tr := TimeRange{From: "asdf", To: "asdf", now: now}
			_, err = tr.ParseFrom()
