...
func (e *CloudWatchExecutor) executeTimeSeriesQuery(ctx context.Context, queryContext *tsdb.TsdbQuery) (*tsdb.Response, error) {
	...
	for i, model := range queryContext.Queries {
		...
		if err != nil {
			results.Results[RefId] = &tsdb.QueryResult{Error: err}
			return results, nil
		}
		query.RefId = RefId
		if query.Id != "" {
		...
		if query.Id == "" && query.Expression != "" {
			results.Results[query.RefId] = &tsdb.QueryResult{Error: fmt.Errorf("Invalid query: id should be set if using expression")}
			return results, nil
		}
		eg.Go(func() error {
			defer func() {
				if err := recover(); err != nil {
					...
					if theErr, ok := err.(error); ok {
						resultChan <- &tsdb.QueryResult{RefId: query.RefId, Error: theErr}
					}
				}
			}()
			...
			if err != nil {
				resultChan <- &tsdb.QueryResult{RefId: query.RefId, Error: err}
				return nil
			}
			resultChan <- queryRes
			return nil
			...
			eg.Go(func() error {
				defer func() {
					if err := recover(); err != nil {
						...
						if theErr, ok := err.(error); ok {
							resultChan <- &tsdb.QueryResult{Error: theErr}
						}
					}
				}()
...
func (e *CloudWatchExecutor) executeQuery(ctx context.Context, query *CloudWatchQuery, queryContext *tsdb.TsdbQuery) (*tsdb.QueryResult, error) {
	...
	params := &cloudwatch.GetMetricStatisticsInput{Namespace: aws.String(query.Namespace), MetricName: aws.String(query.MetricName), Dimensions: query.Dimensions, Period: aws.Int64(int64(query.Period))}
...
func (e *CloudWatchExecutor) executeGetMetricDataQuery(ctx context.Context, region string, queries map[string]*CloudWatchQuery, queryContext *tsdb.TsdbQuery) ([]*tsdb.QueryResult, error) {
	...
	for _, query := range queries {
		if !(len(query.Statistics) == 1 && len(query.ExtendedStatistics) == 0) && !(len(query.Statistics) == 0 && len(query.ExtendedStatistics) == 1) {
	...
	params := &cloudwatch.GetMetricDataInput{StartTime: aws.Time(startTime), EndTime: aws.Time(endTime), ScanBy: aws.String("TimestampAscending")}
	for _, query := range queries {
		if query.HighResolution && (((endTime.Unix() - startTime.Unix()) / int64(query.Period)) > 21600) {
			return queryResponses, errors.New("too long query period")
		}
		mdq := &cloudwatch.MetricDataQuery{Id: aws.String(query.Id), ReturnData: aws.Bool(query.ReturnData)}
		if query.Expression != "" {
			mdq.Expression = aws.String(query.Expression)
		} else {
			mdq.MetricStat = &cloudwatch.MetricStat{Metric: &cloudwatch.Metric{Namespace: aws.String(query.Namespace), MetricName: aws.String(query.MetricName)}, Period: aws.Int64(int64(query.Period))}
			for _, d := range query.Dimensions {
				mdq.MetricStat.Metric.Dimensions = append(mdq.MetricStat.Metric.Dimensions, &cloudwatch.Dimension{Name: d.Name, Value: d.Value})
			}
	...
	for i, r := range mdr {
		...
		query := queries[*r.Id]
		series := tsdb.TimeSeries{Tags: map[string]string{}, Points: make([]tsdb.TimePoint, 0)}
		for _, d := range query.Dimensions {
...
func parseDimensions(model *simplejson.Json) ([]*cloudwatch.Dimension, error) {
	...
	for k, v := range model.Get("dimensions").MustMap() {
		...
		if vv, ok := v.(string); ok {
			result = append(result, &cloudwatch.Dimension{Name: &kk, Value: &vv})
		} else {
