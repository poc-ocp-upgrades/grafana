...
func NewStackdriverExecutor(dsInfo *models.DataSource) (tsdb.TsdbQueryEndpoint, error) {
	...
	return &StackdriverExecutor{httpClient: httpClient, dsInfo: dsInfo}, nil
}
func init() {
	_logClusterCodePath()
	defer _logClusterCodePath()
...
func (e *StackdriverExecutor) buildQueries(tsdbQuery *tsdb.TsdbQuery) ([]*StackdriverQuery, error) {
	...
	for _, query := range tsdbQuery.Queries {
		...
		stackdriverQueries = append(stackdriverQueries, &StackdriverQuery{Target: target, Params: params, RefID: query.RefId, GroupBys: groupBysAsStrings, AliasBy: aliasBy})
	}
...
func (e *StackdriverExecutor) parseResponse(queryRes *tsdb.QueryResult, data StackdriverResponse, query *StackdriverQuery) error {
	...
	for _, series := range data.TimeSeries {
		...
		if series.ValueType != "DISTRIBUTION" {
			...
			queryRes.Series = append(queryRes.Series, &tsdb.TimeSeries{Name: metricName, Points: points})
		} else {
			...
			for i := len(series.Points) - 1; i >= 0; i-- {
				...
				for i := 0; i < len(point.Value.DistributionValue.BucketCounts); i++ {
					...
					if _, ok := buckets[i]; !ok {
						...
						buckets[i] = &tsdb.TimeSeries{Name: formatLegendKeys(series.Metric.Type, defaultMetricName, series.Resource.Type, series.Metric.Labels, series.Resource.Labels, additionalLabels, query), Points: make([]tsdb.TimePoint, 0)}
						if maxKey < i {
							maxKey = i
						}
					}
