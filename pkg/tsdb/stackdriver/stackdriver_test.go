...
func TestStackdriver(t *testing.T) {
	...
	Convey("Stackdriver", t, func() {
		...
		Convey("Parse queries from frontend and build Stackdriver API queries", func() {
			...
			tsdbQuery := &tsdb.TsdbQuery{TimeRange: &tsdb.TimeRange{From: fmt.Sprintf("%v", fromStart.Unix()*1000), To: fmt.Sprintf("%v", fromStart.Add(34*time.Minute).Unix()*1000)}, Queries: []*tsdb.Query{{Model: simplejson.NewFromAny(map[string]interface{}{"metricType": "a/metric/type", "view": "FULL", "aliasBy": "testalias", "type": "timeSeriesQuery"}), RefId: "A"}}}
			...
			Convey("and query has filters", func() {
				tsdbQuery.Queries[0].Model = simplejson.NewFromAny(map[string]interface{}{"metricType": "a/metric/type", "filters": []interface{}{"key", "=", "value", "AND", "key2", "=", "value2"}})
				queries, err := executor.buildQueries(tsdbQuery)
				So(err, ShouldBeNil)
				So(len(queries), ShouldEqual, 1)
			...
			Convey("and alignmentPeriod is set to grafana-auto", func() {
				Convey("and IntervalMs is larger than 60000", func() {
					...
					tsdbQuery.Queries[0].Model = simplejson.NewFromAny(map[string]interface{}{"alignmentPeriod": "grafana-auto", "filters": []interface{}{"key", "=", "value", "AND", "key2", "=", "value2"}})
					queries, err := executor.buildQueries(tsdbQuery)
					So(err, ShouldBeNil)
			...
			Convey("and alignmentPeriod is set to stackdriver-auto", func() {
				Convey("and range is two hours", func() {
					...
					tsdbQuery.Queries[0].Model = simplejson.NewFromAny(map[string]interface{}{"target": "target", "alignmentPeriod": "stackdriver-auto"})
					queries, err := executor.buildQueries(tsdbQuery)
					So(err, ShouldBeNil)
					So(queries[0].Params["aggregation.alignmentPeriod"][0], ShouldEqual, `+60s`)
				})
			...
			Convey("and alignmentPeriod is set in frontend", func() {
				Convey("and alignment period is too big", func() {
					...
					tsdbQuery.Queries[0].Model = simplejson.NewFromAny(map[string]interface{}{"alignmentPeriod": "+360000s"})
					queries, err := executor.buildQueries(tsdbQuery)
					So(err, ShouldBeNil)
					So(queries[0].Params["aggregation.alignmentPeriod"][0], ShouldEqual, `+3600s`)
				})
				Convey("and alignment period is within accepted range", func() {
					tsdbQuery.Queries[0].IntervalMs = 1000
					tsdbQuery.Queries[0].Model = simplejson.NewFromAny(map[string]interface{}{"alignmentPeriod": "+600s"})
					queries, err := executor.buildQueries(tsdbQuery)
					So(err, ShouldBeNil)
			...
			Convey("and query has aggregation mean set", func() {
				tsdbQuery.Queries[0].Model = simplejson.NewFromAny(map[string]interface{}{"metricType": "a/metric/type", "primaryAggregation": "REDUCE_MEAN", "view": "FULL"})
				queries, err := executor.buildQueries(tsdbQuery)
				So(err, ShouldBeNil)
				So(len(queries), ShouldEqual, 1)
				So(queries[0].RefID, ShouldEqual, "A")
			...
			Convey("and query has group bys", func() {
				tsdbQuery.Queries[0].Model = simplejson.NewFromAny(map[string]interface{}{"metricType": "a/metric/type", "primaryAggregation": "REDUCE_NONE", "groupBys": []interface{}{"metric.label.group1", "metric.label.group2"}, "view": "FULL"})
				queries, err := executor.buildQueries(tsdbQuery)
				So(err, ShouldBeNil)
				So(len(queries), ShouldEqual, 1)
				So(queries[0].RefID, ShouldEqual, "A")
