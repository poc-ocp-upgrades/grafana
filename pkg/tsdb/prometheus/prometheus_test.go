...
func TestPrometheus(t *testing.T) {
	...
	Convey("Prometheus", t, func() {
		...
		Convey("converting metric name", func() {
			metric := map[p.LabelName]p.LabelValue{p.LabelName("app"): p.LabelValue("backend"), p.LabelName("device"): p.LabelValue("mobile")}
			query := &PrometheusQuery{LegendFormat: "legend {{app}} {{ device }} {{broken}}"}
			So(formatLegend(metric, query), ShouldEqual, "legend backend mobile {{broken}}")
		})
		Convey("build full serie name", func() {
			metric := map[p.LabelName]p.LabelValue{p.LabelName(p.MetricNameLabel): p.LabelValue("http_request_total"), p.LabelName("app"): p.LabelValue("backend"), p.LabelName("device"): p.LabelValue("mobile")}
			query := &PrometheusQuery{LegendFormat: ""}
		...
		Convey("parsing query model with step", func() {
			...
			queryModels := []*tsdb.Query{{Model: jsonModel}}
