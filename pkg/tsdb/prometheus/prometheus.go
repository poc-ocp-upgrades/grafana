...
type PrometheusExecutor struct{ Transport *http.Transport }
...
func NewPrometheusExecutor(dsInfo *models.DataSource) (tsdb.TsdbQueryEndpoint, error) {
	...
	return &PrometheusExecutor{Transport: transport}, nil
}
...
func (e *PrometheusExecutor) getClient(dsInfo *models.DataSource) (apiv1.API, error) {
	...
	if dsInfo.BasicAuth {
		cfg.RoundTripper = basicAuthTransport{Transport: e.Transport, username: dsInfo.BasicAuthUser, password: dsInfo.BasicAuthPassword}
	}
...
func (e *PrometheusExecutor) Query(ctx context.Context, dsInfo *models.DataSource, tsdbQuery *tsdb.TsdbQuery) (*tsdb.Response, error) {
	...
	for _, query := range queries {
		timeRange := apiv1.Range{Start: query.Start, End: query.End, Step: query.Step}
...
func parseQuery(dsInfo *models.DataSource, queries []*tsdb.Query, queryContext *tsdb.TsdbQuery) ([]*PrometheusQuery, error) {
	...
	for _, queryModel := range queries {
		...
		qs = append(qs, &PrometheusQuery{Expr: expr, Step: step, LegendFormat: format, Start: start, End: end, RefId: queryModel.RefId})
	}
	return qs, nil
}
func parseResponse(value model.Value, query *PrometheusQuery) (*tsdb.QueryResult, error) {
	...
	for _, v := range data {
		series := tsdb.TimeSeries{Name: formatLegend(v.Metric, query), Tags: map[string]string{}}
