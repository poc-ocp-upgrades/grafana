...
var engineCache = engineCacheType{cache: make(map[int64]*xorm.Engine), versions: make(map[int64]int)}
...
var NewSqlQueryEndpoint = func(config *SqlQueryEndpointConfiguration, rowTransformer SqlTableRowTransformer, macroEngine SqlMacroEngine, log log.Logger) (TsdbQueryEndpoint, error) {
	queryEndpoint := sqlQueryEndpoint{rowTransformer: rowTransformer, macroEngine: macroEngine, timeColumnNames: []string{"time"}, log: log}
...
func (e *sqlQueryEndpoint) transformToTable(query *Query, rows *core.Rows, result *QueryResult, tsdbQuery *TsdbQuery) error {
	...
	rowCount := 0
	timeIndex := -1
	table := &Table{Columns: make([]TableColumn, columnCount), Rows: make([]RowValues, 0)}
