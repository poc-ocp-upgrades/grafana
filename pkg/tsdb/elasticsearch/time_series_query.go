...
var newTimeSeriesQuery = func(client es.Client, tsdbQuery *tsdb.TsdbQuery, intervalCalculator tsdb.IntervalCalculator) *timeSeriesQuery {
	return &timeSeriesQuery{client: client, tsdbQuery: tsdbQuery, intervalCalculator: intervalCalculator}
}
	...
	for _, q := range queries {
		...
		if len(q.BucketAggs) == 0 {
			if len(q.Metrics) == 0 || q.Metrics[0].Type != "raw_document" {
				result.Results[q.RefID] = &tsdb.QueryResult{RefId: q.RefID, Error: fmt.Errorf("invalid query, missing metrics and aggregations"), ErrorString: "invalid query, missing metrics and aggregations"}
				continue
			}
			metric := q.Metrics[0]
...
func (p *timeSeriesQueryParser) parse(tsdbQuery *tsdb.TsdbQuery) ([]*Query, error) {
	...
	for _, q := range tsdbQuery.Queries {
		...
		queries = append(queries, &Query{TimeField: timeField, RawQuery: rawQuery, BucketAggs: bucketAggs, Metrics: metrics, Alias: alias, Interval: interval, RefID: q.RefId})
	}
	return queries, nil
}
