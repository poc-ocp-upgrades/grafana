...
func (u *S3Uploader) Upload(ctx context.Context, imageDiskPath string) (string, error) {
	...
	creds := credentials.NewChainCredentials([]credentials.Provider{&credentials.StaticProvider{Value: credentials.Value{AccessKeyID: u.accessKey, SecretAccessKey: u.secretKey}}, &credentials.EnvProvider{}, remoteCredProvider(sess)})
	cfg := &aws.Config{Region: aws.String(u.region), Credentials: creds}
	...
	svc := s3.New(sess, cfg)
	params := &s3.PutObjectInput{Bucket: aws.String(u.bucket), Key: aws.String(key), ACL: aws.String(u.acl), Body: file, ContentType: aws.String("image/png")}
