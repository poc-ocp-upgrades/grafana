...
var (
	changeTypeToSymbol	= map[ChangeType]string{ChangeNil: "", ChangeAdded: "+", ChangeDeleted: "-", ChangeOld: "-", ChangeNew: "+"}
	changeTypeToName	= map[ChangeType]string{ChangeNil: "same", ChangeAdded: "added", ChangeDeleted: "deleted", ChangeOld: "old", ChangeNew: "new"}
)
var (
...
var diffTplFuncs = template.FuncMap{"ctos": func(c ChangeType) string {
...
func (f *JSONFormatter) Format(diff diff.Diff) (result string, err error) {
	...
	if v, ok := f.left.(map[string]interface{}); ok {
		...
		return "", fmt.Errorf("expected map[string]interface{} or []interface{}, got %T", f.left)
	}
	b := &bytes.Buffer{}
...
func (f *JSONFormatter) closeLine() {
	...
	f.Lines = append(f.Lines, &JSONLine{LineNum: f.lineCount, RightLine: rightLine, LeftLine: leftLine, Indent: f.line.indent, Text: s, Change: f.line.change, Key: f.line.key, Val: f.line.val})
}
