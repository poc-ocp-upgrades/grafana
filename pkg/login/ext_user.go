...
func UpsertUser(cmd *m.UpsertUserCommand) error {
	...
	userQuery := &m.GetUserByAuthInfoQuery{AuthModule: extUser.AuthModule, AuthId: extUser.AuthId, UserId: extUser.UserId, Email: extUser.Email, Login: extUser.Login}
	...
	if err != nil {
		...
		if extUser.AuthModule != "" && extUser.AuthId != "" {
			cmd2 := &m.SetAuthInfoCommand{UserId: cmd.Result.Id, AuthModule: extUser.AuthModule, AuthId: extUser.AuthId}
	...
	err = bus.Dispatch(&m.SyncTeamsCommand{User: cmd.Result, ExternalUser: extUser})
...
func syncOrgRoles(user *m.User, extUser *m.ExternalUserInfo) error {
	...
	if _, ok := extUser.OrgRoles[user.OrgId]; !ok {
		...
		return bus.Dispatch(&m.SetUsingOrgCommand{UserId: user.Id, OrgId: user.OrgId})
	}
	return nil
}
