...
var validateLoginAttempts = func(username string) error {
	...
	loginAttemptCountQuery := m.GetUserLoginAttemptCountQuery{Username: username, Since: time.Now().Add(-loginAttemptsWindow)}
...
var saveInvalidLoginAttempt = func(query *m.LoginUserQuery) {
	...
	loginAttemptCommand := m.CreateLoginAttemptCommand{Username: query.Username, IpAddress: query.IpAddress}
