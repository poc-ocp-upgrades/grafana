...
func (a *ldapAuther) Dial() error {
	...
	for _, host := range strings.Split(a.server.Host, " ") {
		...
		if a.server.UseSSL {
			tlsCfg := &tls.Config{InsecureSkipVerify: a.server.SkipVerifySSL, ServerName: host, RootCAs: certPool}
...
func (a *ldapAuther) GetGrafanaUserFor(ctx *m.ReqContext, ldapUser *LdapUserInfo) (*m.User, error) {
	...
	upsertUserCmd := &m.UpsertUserCommand{ReqContext: ctx, ExternalUser: extUser, SignupAllowed: setting.LdapAllowSignup}
...
func (a *ldapAuther) searchForUser(username string) (*LdapUserInfo, error) {
	...
	if a.server.GroupSearchFilter == "" {
		...
		for _, groupSearchBase := range a.server.GroupSearchBaseDNs {
			...
			groupSearchReq := ldap.SearchRequest{BaseDN: groupSearchBase, Scope: ldap.ScopeWholeSubtree, DerefAliases: ldap.NeverDerefAliases, Attributes: []string{groupIdAttribute}, Filter: filter}
	...
	return &LdapUserInfo{DN: searchResult.Entries[0].DN, LastName: getLdapAttr(a.server.Attr.Surname, searchResult), FirstName: getLdapAttr(a.server.Attr.Name, searchResult), Username: getLdapAttr(a.server.Attr.Username, searchResult), Email: getLdapAttr(a.server.Attr.Email, searchResult), MemberOf: memberOf}, nil
}
