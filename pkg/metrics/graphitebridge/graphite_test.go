...
func TestWriteSummary(t *testing.T) {
	...
	b, err := NewBridge(&Config{URL: "localhost:8080", Gatherer: reg, CountersAsDelta: true})
	if err != nil {
...
func TestCounterVec(t *testing.T) {
	...
	apicntVec := prometheus.NewCounterVec(prometheus.CounterOpts{Name: "api_response", Namespace: "grafana", Help: "docstring", ConstLabels: prometheus.Labels{"constname": "constvalue"}}, []string{"labelname"})
	reg := prometheus.NewRegistry()
...
func TestPush(t *testing.T) {
	...
	defer _logClusterCodePath()
	reg := prometheus.NewRegistry()
	cntVec := prometheus.NewCounterVec(prometheus.CounterOpts{Name: "name", Help: "docstring", Namespace: "grafana", ConstLabels: prometheus.Labels{"constname": "constvalue"}}, []string{"labelname"})
	cntVec.WithLabelValues("val1").Inc()
	cntVec.WithLabelValues("val2").Inc()
	reg.MustRegister(cntVec)
	host := "localhost"
	port := ":56789"
	b, err := NewBridge(&Config{URL: host + port, Gatherer: reg, Prefix: "prefix."})
	if err != nil {
		t.Fatalf("error creating bridge: %v", err)
	}
	...
	wants := []string{"prefix.name.constname.constvalue.labelname.val1.count 1", "prefix.name.constname.constvalue.labelname.val2.count 1"}
	select {
	case got := <-nmg.readc:
...
func newMockGraphite(port string) (*mockGraphite, error) {
	...
	return &mockGraphite{readc: readc, errc: errc, Listener: ln}, nil
}
