...
func (sc *scenarioContext) defaultPermissionScenario(pt permissionType, flag permissionFlags) {
	...
	existingPermissions := []*m.DashboardAclInfoDTO{toDto(newEditorRolePermission(defaultDashboardID, m.PERMISSION_EDIT)), toDto(newViewerRolePermission(defaultDashboardID, m.PERMISSION_VIEW))}
...
func (sc *scenarioContext) verifyDuplicatePermissionsShouldNotBeAllowed() {
	...
	Convey(tc, func() {
		p := []*m.DashboardAcl{newDefaultUserPermission(dashboardID, m.PERMISSION_VIEW), newDefaultUserPermission(dashboardID, m.PERMISSION_ADMIN)}
	...
	Convey(tc, func() {
		p := []*m.DashboardAcl{newDefaultTeamPermission(dashboardID, m.PERMISSION_VIEW), newDefaultTeamPermission(dashboardID, m.PERMISSION_ADMIN)}
		sc.updatePermissions = p
		_, err := sc.g.CheckPermissionBeforeUpdate(m.PERMISSION_ADMIN, p)
		if err != ErrGuardianPermissionExists {
			sc.reportFailure(tc, ErrGuardianPermissionExists, err)
	...
	Convey(tc, func() {
		p := []*m.DashboardAcl{newEditorRolePermission(dashboardID, m.PERMISSION_VIEW), newEditorRolePermission(dashboardID, m.PERMISSION_ADMIN)}
		sc.updatePermissions = p
		_, err := sc.g.CheckPermissionBeforeUpdate(m.PERMISSION_ADMIN, p)
		if err != ErrGuardianPermissionExists {
			sc.reportFailure(tc, ErrGuardianPermissionExists, err)
	...
	Convey(tc, func() {
		p := []*m.DashboardAcl{newViewerRolePermission(dashboardID, m.PERMISSION_VIEW), newViewerRolePermission(dashboardID, m.PERMISSION_ADMIN)}
		sc.updatePermissions = p
		_, err := sc.g.CheckPermissionBeforeUpdate(m.PERMISSION_ADMIN, p)
		if err != ErrGuardianPermissionExists {
			sc.reportFailure(tc, ErrGuardianPermissionExists, err)
	...
	Convey(tc, func() {
		p := []*m.DashboardAcl{newAdminRolePermission(dashboardID, m.PERMISSION_ADMIN)}
		sc.updatePermissions = p
		_, err := sc.g.CheckPermissionBeforeUpdate(m.PERMISSION_ADMIN, p)
		if err != ErrGuardianPermissionExists {
			sc.reportFailure(tc, ErrGuardianPermissionExists, err)
...
func (sc *scenarioContext) verifyUpdateDashboardPermissionsShouldBeAllowed(pt permissionType) {
	...
	for _, p := range []m.PermissionType{m.PERMISSION_ADMIN, m.PERMISSION_EDIT, m.PERMISSION_VIEW} {
		...
		Convey(tc, func() {
			...
			switch pt {
			case USER:
				permissionList = []*m.DashboardAcl{newEditorRolePermission(dashboardID, p), newViewerRolePermission(dashboardID, p), newCustomUserPermission(dashboardID, otherUserID, p), newDefaultTeamPermission(dashboardID, p)}
			case TEAM:
				permissionList = []*m.DashboardAcl{newEditorRolePermission(dashboardID, p), newViewerRolePermission(dashboardID, p), newDefaultUserPermission(dashboardID, p), newCustomTeamPermission(dashboardID, otherTeamID, p)}
			case EDITOR, VIEWER:
				permissionList = []*m.DashboardAcl{newEditorRolePermission(dashboardID, p), newViewerRolePermission(dashboardID, p), newDefaultUserPermission(dashboardID, p), newDefaultTeamPermission(dashboardID, p)}
			}
...
func (sc *scenarioContext) verifyUpdateDashboardPermissionsShouldNotBeAllowed(pt permissionType) {
	...
	for _, p := range []m.PermissionType{m.PERMISSION_ADMIN, m.PERMISSION_EDIT, m.PERMISSION_VIEW} {
		...
		Convey(tc, func() {
			permissionList := []*m.DashboardAcl{newEditorRolePermission(dashboardID, p), newViewerRolePermission(dashboardID, p)}
			switch pt {
			case USER:
				permissionList = append(permissionList, []*m.DashboardAcl{newCustomUserPermission(dashboardID, otherUserID, p), newDefaultTeamPermission(dashboardID, p)}...)
			case TEAM:
				permissionList = append(permissionList, []*m.DashboardAcl{newDefaultUserPermission(dashboardID, p), newCustomTeamPermission(dashboardID, otherTeamID, p)}...)
			}
			sc.updatePermissions = permissionList
			ok, err := sc.g.CheckPermissionBeforeUpdate(m.PERMISSION_ADMIN, permissionList)
			if err != nil {
...
func (sc *scenarioContext) verifyUpdateChildDashboardPermissionsShouldBeAllowed(pt permissionType, parentFolderPermission m.PermissionType) {
	...
	for _, p := range []m.PermissionType{m.PERMISSION_ADMIN, m.PERMISSION_EDIT, m.PERMISSION_VIEW} {
		...
		Convey(tc, func() {
			...
			switch pt {
			case USER:
				permissionList = []*m.DashboardAcl{newEditorRolePermission(childDashboardID, p), newViewerRolePermission(childDashboardID, p), newCustomUserPermission(childDashboardID, otherUserID, p), newDefaultTeamPermission(childDashboardID, p)}
			case TEAM:
				permissionList = []*m.DashboardAcl{newEditorRolePermission(childDashboardID, p), newViewerRolePermission(childDashboardID, p), newDefaultUserPermission(childDashboardID, p), newCustomTeamPermission(childDashboardID, otherTeamID, p)}
			case EDITOR:
				permissionList = []*m.DashboardAcl{newViewerRolePermission(childDashboardID, p), newDefaultUserPermission(childDashboardID, p), newDefaultTeamPermission(childDashboardID, p)}
				if p > parentFolderPermission {
					permissionList = append(permissionList, newEditorRolePermission(childDashboardID, p))
				}
			case VIEWER:
				permissionList = []*m.DashboardAcl{newEditorRolePermission(childDashboardID, p), newDefaultUserPermission(childDashboardID, p), newDefaultTeamPermission(childDashboardID, p)}
				if p > parentFolderPermission {
...
func (sc *scenarioContext) verifyUpdateChildDashboardPermissionsWithOverrideShouldBeAllowed(pt permissionType, parentFolderPermission m.PermissionType) {
	...
	for _, p := range []m.PermissionType{m.PERMISSION_ADMIN, m.PERMISSION_EDIT, m.PERMISSION_VIEW} {
		...
		Convey(tc, func() {
			...
			switch pt {
			case USER:
				permissionList = []*m.DashboardAcl{newDefaultUserPermission(childDashboardID, p)}
			case TEAM:
				permissionList = []*m.DashboardAcl{newDefaultTeamPermission(childDashboardID, p)}
			case EDITOR:
				permissionList = []*m.DashboardAcl{newEditorRolePermission(childDashboardID, p)}
			case VIEWER:
				permissionList = []*m.DashboardAcl{newViewerRolePermission(childDashboardID, p)}
			}
			sc.updatePermissions = permissionList
