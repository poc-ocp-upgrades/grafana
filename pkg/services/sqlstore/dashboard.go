...
func saveDashboard(sess *DBSession, cmd *m.SaveDashboardCommand) error {
	...
	dashVersion := &m.DashboardVersion{DashboardId: dash.Id, ParentVersion: parentVersion, RestoredFrom: cmd.RestoredFrom, Version: dash.Version, Created: time.Now(), CreatedBy: dash.UpdatedBy, Message: cmd.Message, Data: dash.Data}
...
func findDashboards(query *search.FindPersistedDashboardsQuery) ([]DashboardSearchProjection, error) {
	...
	sb := NewSearchBuilder(query.SignedInUser, limit, query.Permission).WithTags(query.Tags).WithDashboardIdsIn(query.DashboardIds)
	if query.IsStarred {
		sb.IsStarred()
	}
...
func makeQueryResult(query *search.FindPersistedDashboardsQuery, res []DashboardSearchProjection) {
	...
	for _, item := range res {
		...
		if !exists {
			hit = &search.Hit{Id: item.Id, Uid: item.Uid, Title: item.Title, Uri: "db/" + item.Slug, Url: m.GetDashboardFolderUrl(item.IsFolder, item.Uid, item.Slug), Type: getHitType(item), FolderId: item.FolderId, FolderUid: item.FolderUid, FolderTitle: item.FolderTitle, Tags: []string{}}
			if item.FolderId > 0 {
...
func DeleteDashboard(cmd *m.DeleteDashboardCommand) error {
	...
	return inTransaction(func(sess *DBSession) error {
		...
		deletes := []string{"DELETE FROM dashboard_tag WHERE dashboard_id = ? ", "DELETE FROM star WHERE dashboard_id = ? ", "DELETE FROM dashboard WHERE id = ?", "DELETE FROM playlist_item WHERE type = 'dashboard_by_id' AND value = ?", "DELETE FROM dashboard_version WHERE dashboard_id = ?", "DELETE FROM annotation WHERE dashboard_id = ?", "DELETE FROM dashboard_provisioning WHERE dashboard_id = ?"}
...
func GetDashboardPermissionsForUser(query *m.GetDashboardPermissionsForUserQuery) error {
	...
	if query.OrgRole == m.ROLE_ADMIN {
		...
		for _, d := range query.DashboardIds {
			permissions = append(permissions, &m.DashboardPermissionForUser{DashboardId: d, Permission: m.PERMISSION_ADMIN, PermissionName: m.PERMISSION_ADMIN.String()})
		}
...
func getExistingDashboardByIdOrUidForUpdate(sess *DBSession, cmd *m.ValidateDashboardBeforeSaveCommand) (err error) {
	...
	if (existing.IsFolder && !dash.IsFolder) || (!existing.IsFolder && dash.IsFolder) {
...
func HasEditPermissionInFolders(query *m.HasEditPermissionInFoldersQuery) error {
	...
	type folderCount struct{ Count int64 }
