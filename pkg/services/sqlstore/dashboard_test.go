	...
	Convey("Testing DB", t, func() {
		...
		Convey("Given saved dashboard", func() {
			...
			Convey("Should be able to get dashboard by id", func() {
				query := m.GetDashboardQuery{Id: savedDash.Id, OrgId: 1}
			...
			Convey("Should be able to get dashboard by uid", func() {
				query := m.GetDashboardQuery{Uid: savedDash.Uid, OrgId: 1}
				err := GetDashboard(&query)
				So(err, ShouldBeNil)
				So(query.Result.Title, ShouldEqual, "test dash 23")
				So(query.Result.Slug, ShouldEqual, "test-dash-23")
			...
			Convey("Should be able to delete dashboard", func() {
				...
				err := DeleteDashboard(&m.DeleteDashboardCommand{Id: dash.Id, OrgId: 1})
				So(err, ShouldBeNil)
			})
			Convey("Should retry generation of uid once if it fails.", func() {
				...
				cmd := m.SaveDashboardCommand{OrgId: 1, Dashboard: simplejson.NewFromAny(map[string]interface{}{"title": "new dash 12334", "tags": []interface{}{}})}
				err := SaveDashboard(&cmd)
				So(err, ShouldBeNil)
				generateNewUid = util.GenerateShortUid
			})
			Convey("Should be able to create dashboard", func() {
				cmd := m.SaveDashboardCommand{OrgId: 1, Dashboard: simplejson.NewFromAny(map[string]interface{}{"title": "folderId", "tags": []interface{}{}}), UserId: 100}
				err := SaveDashboard(&cmd)
				So(err, ShouldBeNil)
			...
			Convey("Should be able to update dashboard by id and remove folderId", func() {
				cmd := m.SaveDashboardCommand{OrgId: 1, Dashboard: simplejson.NewFromAny(map[string]interface{}{"id": savedDash.Id, "title": "folderId", "tags": []interface{}{}}), Overwrite: true, FolderId: 2, UserId: 100}
				err := SaveDashboard(&cmd)
				So(err, ShouldBeNil)
				So(cmd.Result.FolderId, ShouldEqual, 2)
				cmd = m.SaveDashboardCommand{OrgId: 1, Dashboard: simplejson.NewFromAny(map[string]interface{}{"id": savedDash.Id, "title": "folderId", "tags": []interface{}{}}), FolderId: 0, Overwrite: true, UserId: 100}
				err = SaveDashboard(&cmd)
				So(err, ShouldBeNil)
				query := m.GetDashboardQuery{Id: savedDash.Id, OrgId: 1}
				err = GetDashboard(&query)
			...
			Convey("Should be able to delete a dashboard folder and its children", func() {
				...
				So(err, ShouldBeNil)
				query := search.FindPersistedDashboardsQuery{OrgId: 1, FolderIds: []int64{savedFolder.Id}, SignedInUser: &m.SignedInUser{}}
				err = SearchDashboards(&query)
				So(err, ShouldBeNil)
				So(len(query.Result), ShouldEqual, 0)
			})
			Convey("Should return error if no dashboard is found for update when dashboard id is greater than zero", func() {
				cmd := m.SaveDashboardCommand{OrgId: 1, Overwrite: true, Dashboard: simplejson.NewFromAny(map[string]interface{}{"id": float64(123412321), "title": "Expect error", "tags": []interface{}{}})}
				err := SaveDashboard(&cmd)
			...
			Convey("Should not return error if no dashboard is found for update when dashboard id is zero", func() {
				cmd := m.SaveDashboardCommand{OrgId: 1, Overwrite: true, Dashboard: simplejson.NewFromAny(map[string]interface{}{"id": 0, "title": "New dash", "tags": []interface{}{}})}
				err := SaveDashboard(&cmd)
				So(err, ShouldBeNil)
			})
			...
			Convey("Should be able to search for dashboard folder", func() {
				query := search.FindPersistedDashboardsQuery{Title: "1 test dash folder", OrgId: 1, SignedInUser: &m.SignedInUser{OrgId: 1, OrgRole: m.ROLE_EDITOR}}
				err := SearchDashboards(&query)
				So(err, ShouldBeNil)
			...
			Convey("Should be able to search for a dashboard folder's children", func() {
				query := search.FindPersistedDashboardsQuery{OrgId: 1, FolderIds: []int64{savedFolder.Id}, SignedInUser: &m.SignedInUser{OrgId: 1, OrgRole: m.ROLE_EDITOR}}
				err := SearchDashboards(&query)
				So(err, ShouldBeNil)
				So(len(query.Result), ShouldEqual, 2)
				hit := query.Result[0]
			...
			Convey("Should be able to search for dashboard by dashboard ids", func() {
				Convey("should be able to find two dashboards by id", func() {
					query := search.FindPersistedDashboardsQuery{DashboardIds: []int64{2, 3}, SignedInUser: &m.SignedInUser{OrgId: 1, OrgRole: m.ROLE_EDITOR}}
					err := SearchDashboards(&query)
					So(err, ShouldBeNil)
					So(len(query.Result), ShouldEqual, 2)
					hit := query.Result[0]
			...
			Convey("Given two dashboards, one is starred dashboard by user 10, other starred by user 1", func() {
				...
				StarDashboard(&m.StarDashboardCommand{DashboardId: starredDash.Id, UserId: 10})
				StarDashboard(&m.StarDashboardCommand{DashboardId: savedDash.Id, UserId: 1})
				Convey("Should be able to search for starred dashboards", func() {
					query := search.FindPersistedDashboardsQuery{SignedInUser: &m.SignedInUser{UserId: 10, OrgId: 1, OrgRole: m.ROLE_EDITOR}, IsStarred: true}
					err := SearchDashboards(&query)
					So(err, ShouldBeNil)
					So(len(query.Result), ShouldEqual, 1)
		...
		Convey("Given a plugin with imported dashboards", func() {
			...
			Convey("Should return imported dashboard", func() {
				query := m.GetDashboardsByPluginIdQuery{PluginId: pluginId, OrgId: 1}
