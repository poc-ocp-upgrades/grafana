...
func CreateTeam(cmd *m.CreateTeamCommand) error {
	...
	return inTransaction(func(sess *DBSession) error {
		...
		team := m.Team{Name: cmd.Name, Email: cmd.Email, OrgId: cmd.OrgId, Created: time.Now(), Updated: time.Now()}
...
func UpdateTeam(cmd *m.UpdateTeamCommand) error {
	...
	return inTransaction(func(sess *DBSession) error {
		...
		team := m.Team{Name: cmd.Name, Email: cmd.Email, Updated: time.Now()}
		sess.MustCols("email")
...
func DeleteTeam(cmd *m.DeleteTeamCommand) error {
	...
	return inTransaction(func(sess *DBSession) error {
		...
		deletes := []string{"DELETE FROM team_member WHERE org_id=? and team_id = ?", "DELETE FROM team WHERE org_id=? and id = ?", "DELETE FROM dashboard_acl WHERE org_id=? and team_id = ?"}
...
func AddTeamMember(cmd *m.AddTeamMemberCommand) error {
	...
	return inTransaction(func(sess *DBSession) error {
		...
		entity := m.TeamMember{OrgId: cmd.OrgId, TeamId: cmd.TeamId, UserId: cmd.UserId, External: cmd.External, Created: time.Now(), Updated: time.Now()}
