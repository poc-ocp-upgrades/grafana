...
func GetPreferencesWithDefaults(query *m.GetPreferencesWithDefaultsQuery) error {
	...
	err := x.Where(filter, params...).OrderBy("user_id ASC, team_id ASC").Find(&prefs)
	if err != nil {
		return err
	}
	res := &m.Preferences{Theme: setting.DefaultTheme, Timezone: "browser", HomeDashboardId: 0}
	for _, p := range prefs {
		if p.Theme != "" {
			res.Theme = p.Theme
		}
...
func SavePreferences(cmd *m.SavePreferencesCommand) error {
	...
	return inTransaction(func(sess *DBSession) error {
		...
		if !exists {
			prefs = m.Preferences{UserId: cmd.UserId, OrgId: cmd.OrgId, TeamId: cmd.TeamId, HomeDashboardId: cmd.HomeDashboardId, Timezone: cmd.Timezone, Theme: cmd.Theme, Created: time.Now(), Updated: time.Now()}
