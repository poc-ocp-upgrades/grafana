...
func TestLoginAttempts(t *testing.T) {
	...
	Convey("Testing Login Attempts DB Access", t, func() {
		...
		err := CreateLoginAttempt(&m.CreateLoginAttemptCommand{Username: user, IpAddress: "192.168.0.1"})
		So(err, ShouldBeNil)
		...
		Convey("Should return a total count of zero login attempts when comparing since beginning of time + 2min and 1s", func() {
			query := m.GetUserLoginAttemptCountQuery{Username: user, Since: timePlusTwoMinutes.Add(time.Second * 1)}
