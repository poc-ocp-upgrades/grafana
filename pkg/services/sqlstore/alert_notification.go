...
func CreateAlertNotificationCommand(cmd *m.CreateAlertNotificationCommand) error {
	...
	return inTransaction(func(sess *DBSession) error {
		...
		alertNotification := &m.AlertNotification{OrgId: cmd.OrgId, Name: cmd.Name, Type: cmd.Type, Settings: cmd.Settings, SendReminder: cmd.SendReminder, DisableResolveMessage: cmd.DisableResolveMessage, Frequency: frequency, Created: time.Now(), Updated: time.Now(), IsDefault: cmd.IsDefault}
...
func SetAlertNotificationStateToPendingCommand(ctx context.Context, cmd *m.SetAlertNotificationStateToPendingCommand) error {
	...
	return withDbSession(ctx, func(sess *DBSession) error {
		...
		res, err := sess.Exec(sql, m.AlertNotificationStatePending, newVersion, timeNow().Unix(), cmd.AlertRuleStateUpdatedVersion, cmd.Id, cmd.Version, cmd.AlertRuleStateUpdatedVersion)
		if err != nil {
			return err
		}
...
func GetOrCreateAlertNotificationState(ctx context.Context, cmd *m.GetOrCreateNotificationStateQuery) error {
	...
	return inTransactionCtx(ctx, func(sess *DBSession) error {
		...
		notificationState := &m.AlertNotificationState{OrgId: cmd.OrgId, AlertId: cmd.AlertId, NotifierId: cmd.NotifierId, State: m.AlertNotificationStateUnknown, UpdatedAt: timeNow().Unix()}
