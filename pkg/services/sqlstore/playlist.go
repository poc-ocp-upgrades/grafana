...
func CreatePlaylist(cmd *m.CreatePlaylistCommand) error {
	...
	for _, item := range cmd.Items {
		playlistItems = append(playlistItems, m.PlaylistItem{PlaylistId: playlist.Id, Type: item.Type, Value: item.Value, Order: item.Order, Title: item.Title})
	}
...
func UpdatePlaylist(cmd *m.UpdatePlaylistCommand) error {
	...
	cmd.Result = &m.PlaylistDTO{Id: playlist.Id, OrgId: playlist.OrgId, Name: playlist.Name, Interval: playlist.Interval}
	...
	for index, item := range cmd.Items {
		playlistItems = append(playlistItems, m.PlaylistItem{PlaylistId: playlist.Id, Type: item.Type, Value: item.Value, Order: index + 1, Title: item.Title})
	}
	_, err = x.Insert(&playlistItems)
	return err
}
