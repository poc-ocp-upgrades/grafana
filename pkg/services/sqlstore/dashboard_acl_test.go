	...
	Convey("Testing DB", t, func() {
		...
		Convey("Given a dashboard folder and a user", func() {
			...
			Convey("When adding dashboard permission with userId and teamId set to 0", func() {
				err := testHelperUpdateDashboardAcl(savedFolder.Id, m.DashboardAcl{OrgId: 1, DashboardId: savedFolder.Id, Permission: m.PERMISSION_EDIT})
			...
			Convey("Given dashboard folder with removed default permissions", func() {
				err := UpdateDashboardAcl(&m.UpdateDashboardAclCommand{DashboardId: savedFolder.Id, Items: []*m.DashboardAcl{}})
				So(err, ShouldBeNil)
			...
			Convey("Given dashboard folder permission", func() {
				err := testHelperUpdateDashboardAcl(savedFolder.Id, m.DashboardAcl{OrgId: 1, UserId: currentUser.Id, DashboardId: savedFolder.Id, Permission: m.PERMISSION_EDIT})
				So(err, ShouldBeNil)
				Convey("When reading dashboard acl should include acl for parent folder", func() {
					query := m.GetDashboardAclInfoListQuery{DashboardId: childDash.Id, OrgId: 1}
					err := GetDashboardAclInfoList(&query)
				...
				Convey("Given child dashboard permission", func() {
					err := testHelperUpdateDashboardAcl(childDash.Id, m.DashboardAcl{OrgId: 1, UserId: currentUser.Id, DashboardId: childDash.Id, Permission: m.PERMISSION_EDIT})
					So(err, ShouldBeNil)
			...
			Convey("Given a team", func() {
				...
				Convey("Should be able to add a user permission for a team", func() {
					err := testHelperUpdateDashboardAcl(savedFolder.Id, m.DashboardAcl{OrgId: 1, TeamId: group1.Result.Id, DashboardId: savedFolder.Id, Permission: m.PERMISSION_EDIT})
					So(err, ShouldBeNil)
					q1 := &m.GetDashboardAclInfoListQuery{DashboardId: savedFolder.Id, OrgId: 1}
					err = GetDashboardAclInfoList(q1)
					So(err, ShouldBeNil)
				...
				Convey("Should be able to update an existing permission for a team", func() {
					err := testHelperUpdateDashboardAcl(savedFolder.Id, m.DashboardAcl{OrgId: 1, TeamId: group1.Result.Id, DashboardId: savedFolder.Id, Permission: m.PERMISSION_ADMIN})
					So(err, ShouldBeNil)
					q3 := &m.GetDashboardAclInfoListQuery{DashboardId: savedFolder.Id, OrgId: 1}
					err = GetDashboardAclInfoList(q3)
					So(err, ShouldBeNil)
