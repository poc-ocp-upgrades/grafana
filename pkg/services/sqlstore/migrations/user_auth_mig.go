...
func addUserAuthMigrations(mg *Migrator) {
	...
	mg.AddMigration("alter user_auth.auth_id to length 190", NewRawSqlMigration("").Postgres("ALTER TABLE user_auth ALTER COLUMN auth_id TYPE VARCHAR(190);").Mysql("ALTER TABLE user_auth MODIFY auth_id VARCHAR(190);"))
}
