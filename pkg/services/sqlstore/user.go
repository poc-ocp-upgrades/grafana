...
func getOrgIdForNewUser(cmd *m.CreateUserCommand, sess *DBSession) (int64, error) {
	...
	if setting.AutoAssignOrg {
		...
		if has {
			...
			if setting.AutoAssignOrgId == 1 {
				...
				return 0, fmt.Errorf("Could not create user: organization id %v does not exist", setting.AutoAssignOrgId)
			}
		}
	} else {
	...
	sess.publishAfterCommit(&events.OrgCreated{Timestamp: org.Created, Id: org.Id, Name: org.Name})
	return org.Id, nil
}
func CreateUser(ctx context.Context, cmd *m.CreateUserCommand) error {
	...
	return inTransactionCtx(ctx, func(sess *DBSession) error {
		...
		user := m.User{Email: cmd.Email, Name: cmd.Name, Login: cmd.Login, Company: cmd.Company, IsAdmin: cmd.IsAdmin, OrgId: orgId, EmailVerified: cmd.EmailVerified, Created: time.Now(), Updated: time.Now(), LastSeenAt: time.Now().AddDate(-10, 0, 0)}
		...
		sess.publishAfterCommit(&events.UserCreated{Timestamp: user.Created, Id: user.Id, Name: user.Name, Login: user.Login, Email: user.Email})
		cmd.Result = user
		if !cmd.SkipOrgSetup {
			orgUser := m.OrgUser{OrgId: orgId, UserId: user.Id, Role: m.ROLE_ADMIN, Created: time.Now(), Updated: time.Now()}
...
func UpdateUser(cmd *m.UpdateUserCommand) error {
	...
	return inTransaction(func(sess *DBSession) error {
		...
		sess.publishAfterCommit(&events.UserUpdated{Timestamp: user.Created, Id: user.Id, Name: user.Name, Login: user.Login, Email: user.Email})
		return nil
	})
}
...
func GetUserProfile(query *m.GetUserProfileQuery) error {
	...
	query.Result = m.UserProfileDTO{Id: user.Id, Name: user.Name, Email: user.Email, Login: user.Login, Theme: user.Theme, IsGrafanaAdmin: user.IsAdmin, OrgId: user.OrgId}
	return err
}
...
func SetUserHelpFlag(cmd *m.SetUserHelpFlagCommand) error {
	...
	defer _logClusterCodePath()
	return inTransaction(func(sess *DBSession) error {
		user := m.User{Id: cmd.UserId, HelpFlags1: cmd.HelpFlags1, Updated: time.Now()}
