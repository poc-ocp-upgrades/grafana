...
func CreateOrg(cmd *m.CreateOrgCommand) error {
	...
	return inTransaction(func(sess *DBSession) error {
		...
		org := m.Org{Name: cmd.Name, Created: time.Now(), Updated: time.Now()}
		if _, err := sess.Insert(&org); err != nil {
			return err
		}
		user := m.OrgUser{OrgId: org.Id, UserId: cmd.UserId, Role: m.ROLE_ADMIN, Created: time.Now(), Updated: time.Now()}
...
func UpdateOrg(cmd *m.UpdateOrgCommand) error {
	...
	return inTransaction(func(sess *DBSession) error {
		...
		org := m.Org{Name: cmd.Name, Updated: time.Now()}
...
func DeleteOrg(cmd *m.DeleteOrgCommand) error {
	...
	return inTransaction(func(sess *DBSession) error {
		...
		deletes := []string{"DELETE FROM star WHERE EXISTS (SELECT 1 FROM dashboard WHERE org_id = ? AND star.dashboard_id = dashboard.id)", "DELETE FROM dashboard_tag WHERE EXISTS (SELECT 1 FROM dashboard WHERE org_id = ? AND dashboard_tag.dashboard_id = dashboard.id)", "DELETE FROM dashboard WHERE org_id = ?", "DELETE FROM api_key WHERE org_id = ?", "DELETE FROM data_source WHERE org_id = ?", "DELETE FROM org_user WHERE org_id = ?", "DELETE FROM org WHERE id = ?", "DELETE FROM temp_user WHERE org_id = ?"}
