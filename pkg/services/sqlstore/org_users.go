...
func AddOrgUser(cmd *m.AddOrgUserCommand) error {
	...
	return inTransaction(func(sess *DBSession) error {
		...
		entity := m.OrgUser{OrgId: cmd.OrgId, UserId: cmd.UserId, Role: cmd.Role, Created: time.Now(), Updated: time.Now()}
...
func RemoveOrgUser(cmd *m.RemoveOrgUserCommand) error {
	...
	return inTransaction(func(sess *DBSession) error {
		...
		deletes := []string{"DELETE FROM org_user WHERE org_id=? and user_id=?", "DELETE FROM dashboard_acl WHERE org_id=? and user_id = ?", "DELETE FROM team_member WHERE org_id=? and user_id = ?"}
