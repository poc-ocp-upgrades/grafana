...
func TestAlertNotificationSQLAccess(t *testing.T) {
	...
	Convey("Testing Alert notification sql access", t, func() {
		...
		Convey("Alert notification state", func() {
			...
			Convey("Get no existing state should create a new state", func() {
				...
				Convey("Update existing state to pending with correct version should update database", func() {
					s := *query.Result
					cmd := models.SetAlertNotificationStateToPendingCommand{Id: s.Id, Version: s.Version, AlertRuleStateUpdatedVersion: s.AlertRuleStateUpdatedVersion}
					...
					Convey("Update existing state to completed should update database", func() {
						s := *query.Result
						setStateCmd := models.SetAlertNotificationStateToCompleteCommand{Id: s.Id, Version: cmd.ResultVersion}
					...
					Convey("Update existing state to completed should update database. regardless of version", func() {
						...
						cmd := models.SetAlertNotificationStateToCompleteCommand{Id: s.Id, Version: unknownVersion}
				...
				Convey("Update existing state to pending with incorrect version should return version mismatch error", func() {
					s := *query.Result
					s.Version = 1000
					cmd := models.SetAlertNotificationStateToPendingCommand{Id: s.NotifierId, Version: s.Version, AlertRuleStateUpdatedVersion: s.AlertRuleStateUpdatedVersion}
					err := SetAlertNotificationStateToPendingCommand(context.Background(), &cmd)
				...
				Convey("Updating existing state to pending with incorrect version since alert rule state update version is higher", func() {
					s := *query.Result
					cmd := models.SetAlertNotificationStateToPendingCommand{Id: s.Id, Version: s.Version, AlertRuleStateUpdatedVersion: 1000}
					err := SetAlertNotificationStateToPendingCommand(context.Background(), &cmd)
					So(err, ShouldBeNil)
					So(cmd.ResultVersion, ShouldEqual, 1)
				})
		...
		Convey("Alert notifications should be empty", func() {
			cmd := &models.GetAlertNotificationsQuery{OrgId: 2, Name: "email"}
			err := GetAlertNotifications(cmd)
			So(err, ShouldBeNil)
			So(cmd.Result, ShouldBeNil)
		})
		Convey("Cannot save alert notifier with send reminder = true", func() {
			cmd := &models.CreateAlertNotificationCommand{Name: "ops", Type: "email", OrgId: 1, SendReminder: true, Settings: simplejson.New()}
		...
		Convey("Cannot update alert notifier with send reminder = false", func() {
			cmd := &models.CreateAlertNotificationCommand{Name: "ops update", Type: "email", OrgId: 1, SendReminder: false, Settings: simplejson.New()}
			err := CreateAlertNotificationCommand(cmd)
			So(err, ShouldBeNil)
			updateCmd := &models.UpdateAlertNotificationCommand{Id: cmd.Result.Id, SendReminder: true}
			Convey("and missing frequency", func() {
		...
		Convey("Can save Alert Notification", func() {
			cmd := &models.CreateAlertNotificationCommand{Name: "ops", Type: "email", OrgId: 1, SendReminder: true, Frequency: "10s", Settings: simplejson.New()}
			err := CreateAlertNotificationCommand(cmd)
			So(err, ShouldBeNil)
			...
			Convey("Can update alert notification", func() {
				newCmd := &models.UpdateAlertNotificationCommand{Name: "NewName", Type: "webhook", OrgId: cmd.Result.OrgId, SendReminder: true, DisableResolveMessage: true, Frequency: "60s", Settings: simplejson.New(), Id: cmd.Result.Id}
			...
			Convey("Can update alert notification to disable sending of reminders", func() {
				newCmd := &models.UpdateAlertNotificationCommand{Name: "NewName", Type: "webhook", OrgId: cmd.Result.OrgId, SendReminder: false, Settings: simplejson.New(), Id: cmd.Result.Id}
				err := UpdateAlertNotification(newCmd)
				So(err, ShouldBeNil)
		...
		Convey("Can search using an array of ids", func() {
			...
			Convey("search", func() {
				query := &models.GetAlertNotificationsToSendQuery{Ids: []int64{cmd1.Result.Id, cmd2.Result.Id, 112341231}, OrgId: 1}
				err := GetAlertNotificationsToSend(query)
				So(err, ShouldBeNil)
				So(len(query.Result), ShouldEqual, 3)
			})
			Convey("all", func() {
				query := &models.GetAllAlertNotificationsQuery{OrgId: 1}
