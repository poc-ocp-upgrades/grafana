...
func TestAlertingDataAccess(t *testing.T) {
	...
	Convey("Testing Alerting data access", t, func() {
		...
		items := []*m.Alert{{PanelId: 1, DashboardId: testDash.Id, OrgId: testDash.OrgId, Name: "Alerting title", Message: "Alerting message", Settings: simplejson.New(), Frequency: 1, EvalData: evalData}}
		cmd := m.SaveAlertsCommand{Alerts: items, DashboardId: testDash.Id, OrgId: 1, UserId: 1}
		err := SaveAlerts(&cmd)
		...
		Convey("Can set new states", func() {
			Convey("new state ok", func() {
				cmd := &m.SetAlertStateCommand{AlertId: 1, State: m.AlertStateOK}
		...
		Convey("Alerts with same dashboard id and panel id should update", func() {
			...
			modifiedCmd := m.SaveAlertsCommand{DashboardId: testDash.Id, OrgId: 1, UserId: 1, Alerts: modifiedItems}
		...
		Convey("Multiple alerts per dashboard", func() {
			multipleItems := []*m.Alert{{DashboardId: testDash.Id, PanelId: 1, Name: "1", OrgId: 1, Settings: simplejson.New()}, {DashboardId: testDash.Id, PanelId: 2, Name: "2", OrgId: 1, Settings: simplejson.New()}, {DashboardId: testDash.Id, PanelId: 3, Name: "3", OrgId: 1, Settings: simplejson.New()}}
		...
		Convey("When dashboard is removed", func() {
			items := []*m.Alert{{PanelId: 1, DashboardId: testDash.Id, Name: "Alerting title", Message: "Alerting message"}}
			cmd := m.SaveAlertsCommand{Alerts: items, DashboardId: testDash.Id, OrgId: 1, UserId: 1}
			SaveAlerts(&cmd)
			err = DeleteDashboard(&m.DeleteDashboardCommand{OrgId: 1, Id: testDash.Id})
			So(err, ShouldBeNil)
...
func insertTestAlert(title string, message string, orgId int64, dashId int64, settings *simplejson.Json) (*m.Alert, error) {
	...
	cmd := m.SaveAlertsCommand{Alerts: items, DashboardId: dashId, OrgId: orgId, UserId: 1}
	err := SaveAlerts(&cmd)
