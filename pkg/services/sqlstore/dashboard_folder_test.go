		...
		Convey("Given one dashboard folder with two dashboards and one dashboard in the root folder", func() {
			...
			Convey("and no acls are set", func() {
				Convey("should return all dashboards", func() {
					query := &search.FindPersistedDashboardsQuery{SignedInUser: &m.SignedInUser{UserId: currentUser.Id, OrgId: 1, OrgRole: m.ROLE_VIEWER}, OrgId: 1, DashboardIds: []int64{folder.Id, dashInRoot.Id}}
			...
			Convey("and acl is set for dashboard folder", func() {
				...
				Convey("when the user is an admin", func() {
					Convey("should be able to access folder", func() {
						query := &search.FindPersistedDashboardsQuery{SignedInUser: &m.SignedInUser{UserId: currentUser.Id, OrgId: 1, OrgRole: m.ROLE_ADMIN}, OrgId: 1, DashboardIds: []int64{folder.Id, dashInRoot.Id}}
						err := SearchDashboards(query)
						So(err, ShouldBeNil)
						So(len(query.Result), ShouldEqual, 2)
						So(query.Result[0].Id, ShouldEqual, folder.Id)
			...
			Convey("and acl is set for dashboard child and folder has all permissions removed", func() {
				...
				Convey("when the user is an admin", func() {
					Convey("should be able to search for child dash and folder", func() {
						query := &search.FindPersistedDashboardsQuery{SignedInUser: &m.SignedInUser{UserId: currentUser.Id, OrgId: 1, OrgRole: m.ROLE_ADMIN}, OrgId: 1, DashboardIds: []int64{folder.Id, dashInRoot.Id, childDash.Id}}
						err := SearchDashboards(query)
						So(err, ShouldBeNil)
			...
			Convey("and acl is set for one dashboard folder", func() {
				...
				Convey("and a dashboard is moved from folder without acl to the folder with an acl", func() {
					...
					Convey("should not return folder with acl or its children", func() {
						query := &search.FindPersistedDashboardsQuery{SignedInUser: &m.SignedInUser{UserId: currentUser.Id, OrgId: 1, OrgRole: m.ROLE_VIEWER}, OrgId: 1, DashboardIds: []int64{folder1.Id, childDash1.Id, childDash2.Id, dashInRoot.Id}}
						err := SearchDashboards(query)
						So(err, ShouldBeNil)
						So(len(query.Result), ShouldEqual, 1)
						So(query.Result[0].Id, ShouldEqual, dashInRoot.Id)
				...
				Convey("and a dashboard is moved from folder with acl to the folder without an acl", func() {
					...
					Convey("should return folder without acl and its children", func() {
						query := &search.FindPersistedDashboardsQuery{SignedInUser: &m.SignedInUser{UserId: currentUser.Id, OrgId: 1, OrgRole: m.ROLE_VIEWER}, OrgId: 1, DashboardIds: []int64{folder2.Id, childDash1.Id, childDash2.Id, dashInRoot.Id}}
						err := SearchDashboards(query)
						So(err, ShouldBeNil)
						So(len(query.Result), ShouldEqual, 4)
		...
		Convey("Given two dashboard folders", func() {
			...
			Convey("Admin users", func() {
				Convey("Should have write access to all dashboard folders in their org", func() {
					query := search.FindPersistedDashboardsQuery{OrgId: 1, SignedInUser: &m.SignedInUser{UserId: adminUser.Id, OrgRole: m.ROLE_ADMIN, OrgId: 1}, Permission: m.PERMISSION_VIEW, Type: "dash-folder"}
					err := SearchDashboards(&query)
					So(err, ShouldBeNil)
					So(len(query.Result), ShouldEqual, 2)
					So(query.Result[0].Id, ShouldEqual, folder1.Id)
				...
				Convey("should have write access to all folders and dashboards", func() {
					query := m.GetDashboardPermissionsForUserQuery{DashboardIds: []int64{folder1.Id, folder2.Id}, OrgId: 1, UserId: adminUser.Id, OrgRole: m.ROLE_ADMIN}
				...
				Convey("should have edit permission in folders", func() {
					query := &m.HasEditPermissionInFoldersQuery{SignedInUser: &m.SignedInUser{UserId: adminUser.Id, OrgId: 1, OrgRole: m.ROLE_ADMIN}}
			...
			Convey("Editor users", func() {
				query := search.FindPersistedDashboardsQuery{OrgId: 1, SignedInUser: &m.SignedInUser{UserId: editorUser.Id, OrgRole: m.ROLE_EDITOR, OrgId: 1}, Permission: m.PERMISSION_EDIT}
				...
				Convey("should have edit access to folders with default ACL", func() {
					query := m.GetDashboardPermissionsForUserQuery{DashboardIds: []int64{folder1.Id, folder2.Id}, OrgId: 1, UserId: editorUser.Id, OrgRole: m.ROLE_EDITOR}
					err := GetDashboardPermissionsForUser(&query)
					So(err, ShouldBeNil)
					So(len(query.Result), ShouldEqual, 2)
					So(query.Result[0].DashboardId, ShouldEqual, folder1.Id)
				...
				Convey("should have edit permission in folders", func() {
					query := &m.HasEditPermissionInFoldersQuery{SignedInUser: &m.SignedInUser{UserId: editorUser.Id, OrgId: 1, OrgRole: m.ROLE_EDITOR}}
					err := HasEditPermissionInFolders(query)
					So(err, ShouldBeNil)
					So(query.Result, ShouldBeTrue)
				})
			...
			Convey("Viewer users", func() {
				query := search.FindPersistedDashboardsQuery{OrgId: 1, SignedInUser: &m.SignedInUser{UserId: viewerUser.Id, OrgRole: m.ROLE_VIEWER, OrgId: 1}, Permission: m.PERMISSION_EDIT}
				...
				Convey("should have view access to folders with default ACL", func() {
					query := m.GetDashboardPermissionsForUserQuery{DashboardIds: []int64{folder1.Id, folder2.Id}, OrgId: 1, UserId: viewerUser.Id, OrgRole: m.ROLE_VIEWER}
					err := GetDashboardPermissionsForUser(&query)
					So(err, ShouldBeNil)
					So(len(query.Result), ShouldEqual, 2)
					So(query.Result[0].DashboardId, ShouldEqual, folder1.Id)
				...
				Convey("should not have edit permission in folders", func() {
					query := &m.HasEditPermissionInFoldersQuery{SignedInUser: &m.SignedInUser{UserId: viewerUser.Id, OrgId: 1, OrgRole: m.ROLE_VIEWER}}
					err := HasEditPermissionInFolders(query)
					So(err, ShouldBeNil)
