	...
	Convey("Testing Account DB Access", t, func() {
		...
		Convey("Given two saved users", func() {
			...
			Convey("Given an added org user", func() {
				cmd := m.AddOrgUserCommand{OrgId: ac1.OrgId, UserId: ac2.Id, Role: m.ROLE_VIEWER}
				err := AddOrgUser(&cmd)
				...
				Convey("Can get organization users with query", func() {
					query := m.GetOrgUsersQuery{OrgId: ac1.OrgId, Query: "ac1"}
					err := GetOrgUsers(&query)
					So(err, ShouldBeNil)
				...
				Convey("Can get organization users with query and limit", func() {
					query := m.GetOrgUsersQuery{OrgId: ac1.OrgId, Query: "ac", Limit: 1}
					err := GetOrgUsers(&query)
					So(err, ShouldBeNil)
					So(len(query.Result), ShouldEqual, 1)
					So(query.Result[0].Email, ShouldEqual, ac1.Email)
				...
				Convey("Given an org user with dashboard permissions", func() {
					...
					So(err, ShouldBeNil)
					ac3 := ac3cmd.Result
					orgUserCmd := m.AddOrgUserCommand{OrgId: ac1.OrgId, UserId: ac3.Id, Role: m.ROLE_VIEWER}
