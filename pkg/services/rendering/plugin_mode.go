...
func (rs *RenderingService) startPlugin(ctx context.Context) error {
	...
	rs.pluginClient = plugin.NewClient(&plugin.ClientConfig{HandshakeConfig: handshakeConfig, Plugins: map[string]plugin.Plugin{plugins.Renderer.Id: &pluginModel.RendererPluginImpl{}}, Cmd: exec.Command(fullpath), AllowedProtocols: []plugin.Protocol{plugin.ProtocolGRPC}, Logger: plugins.LogWrapper{Logger: rs.log}})
...
func (rs *RenderingService) renderViaPlugin(ctx context.Context, opts Opts) (*RenderResult, error) {
	...
	rsp, err := rs.grpcPlugin.Render(ctx, &pluginModel.RenderRequest{Url: rs.getURL(opts.Path), Width: int32(opts.Width), Height: int32(opts.Height), FilePath: pngPath, Timeout: int32(opts.Timeout.Seconds()), RenderKey: rs.getRenderKey(opts.OrgId, opts.UserId, opts.OrgRole), Encoding: opts.Encoding, Timezone: isoTimeOffsetToPosixTz(opts.Timezone), Domain: rs.domain})
	if err != nil {
		return nil, err
	}
	if rsp.Error != "" {
