...
func (n *notificationService) sendAndMarkAsComplete(evalContext *EvalContext, notifierState *notifierState) error {
	...
	cmd := &m.SetAlertNotificationStateToCompleteCommand{Id: notifierState.state.Id, Version: notifierState.state.Version}
...
func (n *notificationService) sendNotification(evalContext *EvalContext, notifierState *notifierState) error {
	...
	defer _logClusterCodePath()
	if !evalContext.IsTestRun {
		setPendingCmd := &m.SetAlertNotificationStateToPendingCommand{Id: notifierState.state.Id, Version: notifierState.state.Version, AlertRuleStateUpdatedVersion: evalContext.Rule.StateChanges}
...
func (n *notificationService) uploadImage(context *EvalContext) (err error) {
	...
	renderOpts := rendering.Opts{Width: 1000, Height: 500, Timeout: alertTimeout / 2, OrgId: context.Rule.OrgId, OrgRole: m.ROLE_ADMIN, ConcurrentLimit: setting.AlertingRenderLimit}
...
func (n *notificationService) getNeededNotifiers(orgId int64, notificationIds []int64, evalContext *EvalContext) (notifierStateSlice, error) {
	...
	for _, notification := range query.Result {
		...
		query := &m.GetOrCreateNotificationStateQuery{NotifierId: notification.Id, AlertId: evalContext.Rule.Id, OrgId: evalContext.Rule.OrgId}
		...
		if not.ShouldNotify(evalContext.Ctx, evalContext, query.Result) {
			result = append(result, &notifierState{notifier: not, state: query.Result})
		}
	}
	return result, nil
}
