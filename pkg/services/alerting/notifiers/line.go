...
func NewLINENotifier(model *m.AlertNotification) (alerting.Notifier, error) {
	...
	return &LineNotifier{NotifierBase: NewNotifierBase(model), Token: token, log: log.New("alerting.notifier.line")}, nil
}
...
func (this *LineNotifier) createAlert(evalContext *alerting.EvalContext) error {
	...
	cmd := &m.SendWebhookSync{Url: lineNotifyUrl, HttpMethod: "POST", HttpHeader: map[string]string{"Authorization": fmt.Sprintf("Bearer %s", this.Token), "Content-Type": "application/x-www-form-urlencoded;charset=UTF-8"}, Body: form.Encode()}
