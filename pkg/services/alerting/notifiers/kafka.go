...
func NewKafkaNotifier(model *m.AlertNotification) (alerting.Notifier, error) {
	...
	return &KafkaNotifier{NotifierBase: NewNotifierBase(model), Endpoint: endpoint, Topic: topic, log: log.New("alerting.notifier.kafka")}, nil
}
...
func (this *KafkaNotifier) Notify(evalContext *alerting.EvalContext) error {
	...
	cmd := &m.SendWebhookSync{Url: topicUrl, Body: string(body), HttpMethod: "POST", HttpHeader: map[string]string{"Content-Type": "application/vnd.kafka.json.v2+json", "Accept": "application/vnd.kafka.v2+json"}}
