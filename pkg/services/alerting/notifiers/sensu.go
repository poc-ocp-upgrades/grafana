...
func NewSensuNotifier(model *m.AlertNotification) (alerting.Notifier, error) {
	...
	return &SensuNotifier{NotifierBase: NewNotifierBase(model), Url: url, User: model.Settings.Get("username").MustString(), Source: model.Settings.Get("source").MustString(), Password: model.Settings.Get("password").MustString(), Handler: model.Settings.Get("handler").MustString(), log: log.New("alerting.notifier.sensu")}, nil
}
...
func (this *SensuNotifier) Notify(evalContext *alerting.EvalContext) error {
	...
	cmd := &m.SendWebhookSync{Url: this.Url, User: this.User, Password: this.Password, Body: string(body), HttpMethod: "POST"}
