...
func NewPagerdutyNotifier(model *m.AlertNotification) (alerting.Notifier, error) {
	...
	return &PagerdutyNotifier{NotifierBase: NewNotifierBase(model), Key: key, AutoResolve: autoResolve, log: log.New("alerting.notifier.pagerduty")}, nil
}
...
func (this *PagerdutyNotifier) Notify(evalContext *alerting.EvalContext) error {
	...
	cmd := &m.SendWebhookSync{Url: pagerdutyEventApiUrl, Body: string(body), HttpMethod: "POST", HttpHeader: map[string]string{"Content-Type": "application/json"}}
