...
func NewDiscordNotifier(model *m.AlertNotification) (alerting.Notifier, error) {
	...
	return &DiscordNotifier{NotifierBase: NewNotifierBase(model), WebhookURL: url, log: log.New("alerting.notifier.discord")}, nil
}
...
func (this *DiscordNotifier) Notify(evalContext *alerting.EvalContext) error {
	...
	for _, evt := range evalContext.EvalMatches {
		fields = append(fields, map[string]interface{}{"name": evt.Metric, "value": evt.Value.FullString(), "inline": true})
	}
	footer := map[string]interface{}{"text": "Grafana v" + setting.BuildVersion, "icon_url": "https://grafana.com/assets/img/fav32.png"}
	...
	if evalContext.ImagePublicUrl != "" {
		image = map[string]interface{}{"url": evalContext.ImagePublicUrl}
		embed.Set("image", image)
	} else {
		image = map[string]interface{}{"url": "attachment://graph.png"}
		embed.Set("image", image)
		embeddedImage = true
	}
	...
	cmd := &m.SendWebhookSync{Url: this.WebhookURL, Body: string(body), HttpMethod: "POST", ContentType: content_type}
