...
func NewOpsGenieNotifier(model *m.AlertNotification) (alerting.Notifier, error) {
	...
	return &OpsGenieNotifier{NotifierBase: NewNotifierBase(model), ApiKey: apiKey, ApiUrl: apiUrl, AutoClose: autoClose, log: log.New("alerting.notifier.opsgenie")}, nil
}
...
func (this *OpsGenieNotifier) createAlert(evalContext *alerting.EvalContext) error {
	...
	cmd := &m.SendWebhookSync{Url: this.ApiUrl, Body: string(body), HttpMethod: "POST", HttpHeader: map[string]string{"Content-Type": "application/json", "Authorization": fmt.Sprintf("GenieKey %s", this.ApiKey)}}
...
func (this *OpsGenieNotifier) closeAlert(evalContext *alerting.EvalContext) error {
	...
	bodyJSON.Set("source", "Grafana")
	body, _ := bodyJSON.MarshalJSON()
	cmd := &m.SendWebhookSync{Url: fmt.Sprintf("%s/alertId-%d/close?identifierType=alias", this.ApiUrl, evalContext.Rule.Id), Body: string(body), HttpMethod: "POST", HttpHeader: map[string]string{"Content-Type": "application/json", "Authorization": fmt.Sprintf("GenieKey %s", this.ApiKey)}}
	if err := bus.DispatchCtx(evalContext.Ctx, cmd); err != nil {
		this.log.Error("Failed to send notification to OpsGenie", "error", err, "body", string(body))
		return err
	}
