...
func NewTelegramNotifier(model *m.AlertNotification) (alerting.Notifier, error) {
	...
	return &TelegramNotifier{NotifierBase: NewNotifierBase(model), BotToken: botToken, ChatID: chatId, UploadImage: uploadImage, log: log.New("alerting.notifier.telegram")}, nil
}
...
func (this *TelegramNotifier) generateTelegramCmd(message string, messageField string, apiAction string, extraConf func(writer *multipart.Writer)) *m.SendWebhookSync {
	...
	cmd := &m.SendWebhookSync{Url: url, Body: body.String(), HttpMethod: "POST", HttpHeader: map[string]string{"Content-Type": w.FormDataContentType()}}
	return cmd
}
