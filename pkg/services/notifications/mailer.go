...
func (ns *NotificationService) createDialer() (*gomail.Dialer, error) {
	...
	tlsconfig := &tls.Config{InsecureSkipVerify: ns.Cfg.Smtp.SkipVerify, ServerName: host}
...
func (ns *NotificationService) buildEmailMessage(cmd *m.SendEmailCommand) (*Message, error) {
	...
	return &Message{To: cmd.To, From: fmt.Sprintf("%s <%s>", ns.Cfg.Smtp.FromName, ns.Cfg.Smtp.FromAddress), Subject: subject, Body: buffer.String(), EmbededFiles: cmd.EmbededFiles}, nil
}
