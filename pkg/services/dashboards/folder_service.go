...
var NewFolderService = func(orgId int64, user *models.SignedInUser) FolderService {
	return &dashboardServiceImpl{orgId: orgId, user: user}
}
...
func (dr *dashboardServiceImpl) GetFolders(limit int) ([]*models.Folder, error) {
	...
	searchQuery := search.Query{SignedInUser: dr.user, DashboardIds: make([]int64, 0), FolderIds: make([]int64, 0), Limit: limit, OrgId: dr.orgId, Type: "dash-folder", Permission: models.PERMISSION_VIEW}
	if err := bus.Dispatch(&searchQuery); err != nil {
		return nil, err
	}
	folders := make([]*models.Folder, 0)
	for _, hit := range searchQuery.Result {
		folders = append(folders, &models.Folder{Id: hit.Id, Uid: hit.Uid, Title: hit.Title})
	}
	return folders, nil
}
...
func (dr *dashboardServiceImpl) CreateFolder(cmd *models.CreateFolderCommand) error {
	...
	dto := &SaveDashboardDTO{Dashboard: dashFolder, OrgId: dr.orgId, User: dr.user}
...
func (dr *dashboardServiceImpl) UpdateFolder(existingUid string, cmd *models.UpdateFolderCommand) error {
	...
	dto := &SaveDashboardDTO{Dashboard: dashFolder, OrgId: dr.orgId, User: dr.user, Overwrite: cmd.Overwrite}
	saveDashboardCmd, err := dr.buildSaveDashboardCommand(dto, false, false)
	if err != nil {
		return toFolderError(err)
	}
