...
func (hs *HTTPServer) GetPluginList(c *m.ReqContext) Response {
	...
	for _, pluginDef := range plugins.Plugins {
		...
		listItem := dtos.PluginListItem{Id: pluginDef.Id, Name: pluginDef.Name, Type: pluginDef.Type, Info: &pluginDef.Info, LatestVersion: pluginDef.GrafanaNetVersion, HasUpdate: pluginDef.GrafanaNetHasUpdate, DefaultNavUrl: pluginDef.DefaultNavUrl, State: pluginDef.State}
...
func GetPluginSettingByID(c *m.ReqContext) Response {
	...
	dto := &dtos.PluginSetting{Type: def.Type, Id: def.Id, Name: def.Name, Info: &def.Info, Dependencies: &def.Dependencies, Includes: def.Includes, BaseUrl: def.BaseUrl, Module: def.Module, DefaultNavUrl: def.DefaultNavUrl, LatestVersion: def.GrafanaNetVersion, HasUpdate: def.GrafanaNetHasUpdate, State: def.State}
