...
func GetUserByLoginOrEmail(c *m.ReqContext) Response {
	...
	user := query.Result
	result := m.UserProfileDTO{Id: user.Id, Name: user.Name, Email: user.Email, Login: user.Login, Theme: user.Theme, IsGrafanaAdmin: user.IsAdmin, OrgId: user.OrgId}
...
func SetHelpFlag(c *m.ReqContext) Response {
	...
	cmd := m.SetUserHelpFlagCommand{UserId: c.UserId, HelpFlags1: *bitmask}
	if err := bus.Dispatch(&cmd); err != nil {
