...
func GetDataSources(c *m.ReqContext) Response {
	...
	for _, ds := range query.Result {
		dsItem := dtos.DataSourceListItemDTO{OrgId: ds.OrgId, Id: ds.Id, Name: ds.Name, Url: ds.Url, Type: ds.Type, Access: ds.Access, Password: ds.Password, Database: ds.Database, User: ds.User, BasicAuth: ds.BasicAuth, IsDefault: ds.IsDefault, JsonData: ds.JsonData, ReadOnly: ds.ReadOnly}
...
func UpdateDataSource(c *m.ReqContext, cmd m.UpdateDataSourceCommand) Response {
	...
	query := m.GetDataSourceByIdQuery{Id: cmd.Id, OrgId: c.OrgId}
	if err := bus.Dispatch(&query); err != nil {
		if err == m.ErrDataSourceNotFound {
			return Error(404, "Data source not found", nil)
		}
	...
	return JSON(200, util.DynMap{"message": "Datasource updated", "id": cmd.Id, "name": cmd.Name, "datasource": dtos})
}
