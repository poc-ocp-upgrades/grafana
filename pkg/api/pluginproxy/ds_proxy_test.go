...
func TestDSRouteRule(t *testing.T) {
	...
	Convey("DataSourceProxy", t, func() {
		Convey("Plugin with routes", func() {
			...
			ds := &m.DataSource{JsonData: simplejson.NewFromAny(map[string]interface{}{"clientId": "asd", "dynamicUrl": "https://dynamic.grafana.com"}), SecureJsonData: map[string][]byte{"key": key}}
			req, _ := http.NewRequest("GET", "http://localhost/asd", nil)
			ctx := &m.ReqContext{Context: &macaron.Context{Req: macaron.Request{Request: req}}, SignedInUser: &m.SignedInUser{OrgRole: m.ROLE_EDITOR}}
		...
		Convey("Plugin with multiple routes for token auth", func() {
			plugin := &plugins.DataSourcePlugin{Routes: []*plugins.AppPluginRoute{{Path: "pathwithtoken1", Url: "https://api.nr1.io/some/path", TokenAuth: &plugins.JwtTokenAuth{Url: "https://login.server.com/{{.JsonData.tenantId}}/oauth2/token", Params: map[string]string{"grant_type": "client_credentials", "client_id": "{{.JsonData.clientId}}", "client_secret": "{{.SecureJsonData.clientSecret}}", "resource": "https://api.nr1.io"}}}, {Path: "pathwithtoken2", Url: "https://api.nr2.io/some/path", TokenAuth: &plugins.JwtTokenAuth{Url: "https://login.server.com/{{.JsonData.tenantId}}/oauth2/token", Params: map[string]string{"grant_type": "client_credentials", "client_id": "{{.JsonData.clientId}}", "client_secret": "{{.SecureJsonData.clientSecret}}", "resource": "https://api.nr2.io"}}}}}
			setting.SecretKey = "password"
			key, _ := util.Encrypt([]byte("123"), "password")
			ds := &m.DataSource{JsonData: simplejson.NewFromAny(map[string]interface{}{"clientId": "asd", "tenantId": "mytenantId"}), SecureJsonData: map[string][]byte{"clientSecret": key}}
			req, _ := http.NewRequest("GET", "http://localhost/asd", nil)
			ctx := &m.ReqContext{Context: &macaron.Context{Req: macaron.Request{Request: req}}, SignedInUser: &m.SignedInUser{OrgRole: m.ROLE_EDITOR}}
		...
		Convey("When proxying InfluxDB", func() {
			plugin := &plugins.DataSourcePlugin{}
			ds := &m.DataSource{Type: m.DS_INFLUXDB_08, Url: "http://influxdb:8083", Database: "site", User: "user", Password: "password"}
			ctx := &m.ReqContext{}
		...
		Convey("When proxying a data source with no keepCookies specified", func() {
			...
			ds := &m.DataSource{Type: m.DS_GRAPHITE, Url: "http://graphite:8086", JsonData: json}
			ctx := &m.ReqContext{}
			proxy := NewDataSourceProxy(ds, plugin, ctx, "")
		...
		Convey("When proxying a data source with custom headers specified", func() {
			...
			ds := &m.DataSource{Type: m.DS_PROMETHEUS, Url: "http://prometheus:9090", JsonData: simplejson.NewFromAny(map[string]interface{}{"httpHeaderName1": "Authorization"}), SecureJsonData: map[string][]byte{"httpHeaderValue1": encryptedData}}
			ctx := &m.ReqContext{}
			proxy := NewDataSourceProxy(ds, plugin, ctx, "")
			requestURL, _ := url.Parse("http://grafana.com/sub")
			req := http.Request{URL: requestURL, Header: make(http.Header)}
		...
		Convey("When proxying a custom datasource", func() {
			plugin := &plugins.DataSourcePlugin{}
			ds := &m.DataSource{Type: "custom-datasource", Url: "http://host/root/"}
			ctx := &m.ReqContext{}
...
type httpClientStub struct{ fakeBody []byte }
