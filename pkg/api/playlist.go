...
func SearchPlaylists(c *m.ReqContext) Response {
	...
	searchQuery := m.GetPlaylistsQuery{Name: query, Limit: limit, OrgId: c.OrgId}
...
func GetPlaylist(c *m.ReqContext) Response {
	...
	dto := &m.PlaylistDTO{Id: cmd.Result.Id, Name: cmd.Result.Name, Interval: cmd.Result.Interval, OrgId: cmd.Result.OrgId, Items: playlistDTOs}
	return JSON(200, dto)
}
func LoadPlaylistItemDTOs(id int64) ([]m.PlaylistItemDTO, error) {
	...
	for _, item := range playlistitems {
		playlistDTOs = append(playlistDTOs, m.PlaylistItemDTO{Id: item.Id, PlaylistId: item.PlaylistId, Type: item.Type, Value: item.Value, Order: item.Order, Title: item.Title})
	}
