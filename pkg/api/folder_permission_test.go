...
func TestFolderPermissionApiEndpoint(t *testing.T) {
	...
	Convey("Folder permissions test", t, func() {
		Convey("Given folder not exists", func() {
			...
			cmd := dtos.UpdateDashboardAclCommand{Items: []dtos.DashboardAclUpdateItem{{UserId: 1000, Permission: m.PERMISSION_ADMIN}}}
		...
		Convey("Given user has no admin permissions", func() {
			...
			mock := &fakeFolderService{GetFolderByUIDResult: &m.Folder{Id: 1, Uid: "uid", Title: "Folder"}}
			origNewFolderService := dashboards.NewFolderService
			mockFolderService(mock)
			loggedInUserScenarioWithRole("When calling GET on", "GET", "/api/folders/uid/permissions", "/api/folders/:uid/permissions", m.ROLE_EDITOR, func(sc *scenarioContext) {
				callGetFolderPermissions(sc)
		...
		Convey("When trying to update permissions with duplicate permissions", func() {
			origNewGuardian := guardian.New
			guardian.MockDashboardGuardian(&guardian.FakeDashboardGuardian{CanAdminValue: true, CheckPermissionBeforeUpdateValue: false, CheckPermissionBeforeUpdateError: guardian.ErrGuardianPermissionExists})
			mock := &fakeFolderService{GetFolderByUIDResult: &m.Folder{Id: 1, Uid: "uid", Title: "Folder"}}
			origNewFolderService := dashboards.NewFolderService
			mockFolderService(mock)
			cmd := dtos.UpdateDashboardAclCommand{Items: []dtos.DashboardAclUpdateItem{{UserId: 1000, Permission: m.PERMISSION_ADMIN}}}
		...
		Convey("When trying to override inherited permissions with lower presedence", func() {
			origNewGuardian := guardian.New
			guardian.MockDashboardGuardian(&guardian.FakeDashboardGuardian{CanAdminValue: true, CheckPermissionBeforeUpdateValue: false, CheckPermissionBeforeUpdateError: guardian.ErrGuardianOverride})
			mock := &fakeFolderService{GetFolderByUIDResult: &m.Folder{Id: 1, Uid: "uid", Title: "Folder"}}
			origNewFolderService := dashboards.NewFolderService
			mockFolderService(mock)
			cmd := dtos.UpdateDashboardAclCommand{Items: []dtos.DashboardAclUpdateItem{{UserId: 1000, Permission: m.PERMISSION_ADMIN}}}
