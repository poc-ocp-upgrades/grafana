...
func GetDashboard(c *m.ReqContext) Response {
	...
	dto := dtos.DashboardFullWithMeta{Dashboard: dash.Data, Meta: meta}
...
func PostDashboard(c *m.ReqContext, cmd m.SaveDashboardCommand) Response {
	...
	dashItem := &dashboards.SaveDashboardDTO{Dashboard: dash, Message: cmd.Message, OrgId: c.OrgId, User: c.SignedInUser, Overwrite: cmd.Overwrite}
	dashboard, err := dashboards.NewService().SaveDashboard(dashItem)
	if err == m.ErrDashboardTitleEmpty || err == m.ErrDashboardWithSameNameAsFolder || err == m.ErrDashboardFolderWithSameNameAsDashboard || err == m.ErrDashboardTypeMismatch || err == m.ErrDashboardInvalidUid || err == m.ErrDashboardUidToLong || err == m.ErrDashboardWithSameUIDExists || err == m.ErrFolderNotFound || err == m.ErrDashboardFolderCannotHaveParent || err == m.ErrDashboardFolderNameExists || err == m.ErrDashboardCannotSaveProvisionedDashboard {
	...
	return JSON(200, util.DynMap{"status": "success", "slug": dashboard.Slug, "version": dashboard.Version, "id": dashboard.Id, "uid": dashboard.Uid, "url": dashboard.GetUrl()})
}
...
func GetDashboardVersions(c *m.ReqContext) Response {
	...
	query := m.GetDashboardVersionsQuery{OrgId: c.OrgId, DashboardId: dashID, Limit: c.QueryInt("limit"), Start: c.QueryInt("start")}
	if err := bus.Dispatch(&query); err != nil {
...
func GetDashboardVersion(c *m.ReqContext) Response {
	...
	query := m.GetDashboardVersionQuery{OrgId: c.OrgId, DashboardId: dashID, Version: c.ParamsInt(":id")}
	if err := bus.Dispatch(&query); err != nil {
	...
	dashVersionMeta := &m.DashboardVersionMeta{DashboardVersion: *query.Result, CreatedBy: creator}
