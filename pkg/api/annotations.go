...
type CreateAnnotationError struct{ message string }
...
func PostAnnotation(c *m.ReqContext, cmd dtos.PostAnnotationsCmd) Response {
	...
	item := annotations.Item{OrgId: c.OrgId, UserId: c.UserId, DashboardId: cmd.DashboardId, PanelId: cmd.PanelId, Epoch: cmd.Time, Text: cmd.Text, Data: cmd.Data, Tags: cmd.Tags}
	...
	if cmd.IsRegion {
		...
		return JSON(200, util.DynMap{"message": "Annotation added", "id": startID, "endId": item.Id})
	}
	return JSON(200, util.DynMap{"message": "Annotation added", "id": startID})
}
...
func PostGraphiteAnnotation(c *m.ReqContext, cmd dtos.PostGraphiteAnnotationsCmd) Response {
	...
	item := annotations.Item{OrgId: c.OrgId, UserId: c.UserId, Epoch: cmd.When * 1000, Text: text, Tags: tagsArray}
	if err := repo.Save(&item); err != nil {
		return Error(500, "Failed to save Graphite annotation", err)
	}
	return JSON(200, util.DynMap{"message": "Graphite annotation added", "id": item.Id})
}
func UpdateAnnotation(c *m.ReqContext, cmd dtos.UpdateAnnotationsCmd) Response {
	...
	item := annotations.Item{OrgId: c.OrgId, UserId: c.UserId, Id: annotationID, Epoch: cmd.Time, Text: cmd.Text, Tags: cmd.Tags}
	if err := repo.Update(&item); err != nil {
...
func DeleteAnnotationByID(c *m.ReqContext) Response {
	...
	err := repo.Delete(&annotations.DeleteParams{OrgId: c.OrgId, Id: annotationID})
	if err != nil {
...
func DeleteAnnotationRegion(c *m.ReqContext) Response {
	...
	err := repo.Delete(&annotations.DeleteParams{OrgId: c.OrgId, RegionId: regionID})
	if err != nil {
