...
func GetAlertStatesForDashboard(c *m.ReqContext) Response {
	...
	query := m.GetAlertStatesForDashboardQuery{OrgId: c.OrgId, DashboardId: c.QueryInt64("dashboardId")}
	if err := bus.Dispatch(&query); err != nil {
...
func GetAlerts(c *m.ReqContext) Response {
	...
	if dashboardQuery != "" || len(dashboardTags) > 0 || len(stringFolderIDs) > 0 {
		...
		searchQuery := search.Query{Title: dashboardQuery, Tags: dashboardTags, SignedInUser: c.SignedInUser, Limit: 1000, OrgId: c.OrgId, DashboardIds: dashboardIDs, Type: string(search.DashHitDB), FolderIds: folderIDs, Permission: m.PERMISSION_VIEW}
	...
	query := m.GetAlertsQuery{OrgId: c.OrgId, DashboardIDs: dashboardIDs, PanelId: c.QueryInt64("panelId"), Limit: c.QueryInt64("limit"), User: c.SignedInUser, Query: c.Query("query")}
...
func AlertTest(c *m.ReqContext, dto dtos.AlertTestCommand) Response {
	...
	backendCmd := alerting.AlertTestCommand{OrgId: c.OrgId, Dashboard: dto.Dashboard, PanelId: dto.PanelId, User: c.SignedInUser}
	...
	dtoRes := &dtos.AlertTestResult{Firing: res.Firing, ConditionEvals: res.ConditionEvals, State: res.Rule.State}
	if res.Error != nil {
...
func PauseAlert(c *m.ReqContext, dto dtos.PauseAlertCommand) Response {
	...
	cmd := m.PauseAlertCommand{OrgId: c.OrgId, AlertIds: []int64{alertID}, Paused: dto.Paused}
	if err := bus.Dispatch(&cmd); err != nil {
	...
	result := map[string]interface{}{"alertId": alertID, "state": response, "message": "Alert " + pausedState}
	return JSON(200, result)
}
func PauseAllAlerts(c *m.ReqContext, dto dtos.PauseAllAlertsCommand) Response {
	...
	result := map[string]interface{}{"state": response, "message": "alerts " + pausedState, "alertsAffected": updateCmd.ResultCount}
	return JSON(200, result)
}
