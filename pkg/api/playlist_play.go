...
func populateDashboardsByID(dashboardByIDs []int64, dashboardIDOrder map[int64]int) (dtos.PlaylistDashboardsSlice, error) {
	...
	if len(dashboardByIDs) > 0 {
		...
		for _, item := range dashboardQuery.Result {
			result = append(result, dtos.PlaylistDashboard{Id: item.Id, Slug: item.Slug, Title: item.Title, Uri: "db/" + item.Slug, Order: dashboardIDOrder[item.Id]})
		}
	}
	return result, nil
}
	...
	for _, tag := range dashboardByTag {
		...
		if err := bus.Dispatch(&searchQuery); err == nil {
			for _, item := range searchQuery.Result {
				result = append(result, dtos.PlaylistDashboard{Id: item.Id, Title: item.Title, Uri: item.Uri, Order: dashboardTagOrder[tag]})
			}
		}
	}
	return result
