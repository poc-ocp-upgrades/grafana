...
func (hs *HTTPServer) getFrontendSettingsMap(c *m.ReqContext) (map[string]interface{}, error) {
	...
	if c.OrgId != 0 {
		...
		dsFilterQuery := m.DatasourcesPermissionFilterQuery{User: c.SignedInUser, Datasources: query.Result}
	...
	for _, ds := range orgDataSources {
		...
		var dsMap = map[string]interface{}{"id": ds.Id, "type": ds.Type, "name": ds.Name, "url": url}
	...
	for _, ds := range plugins.DataSources {
		if ds.BuiltIn {
			datasources[ds.Name] = map[string]interface{}{"type": ds.Type, "name": ds.Name, "meta": plugins.DataSources[ds.Id]}
		}
	}
