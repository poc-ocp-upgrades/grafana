...
func initContextWithAuthProxy(ctx *m.ReqContext, orgID int64) bool {
	...
	if sessProxyValue != nil && sessProxyValue.(string) == proxyHeaderValue && getRequestUserId(ctx) > 0 {
		if setting.LdapEnabled {
			syncQuery := &m.LoginUserQuery{ReqContext: ctx, Username: proxyHeaderValue}
		...
		extUser := &m.ExternalUserInfo{AuthModule: "authproxy", AuthId: proxyHeaderValue}
		...
		cmd := &m.UpsertUserCommand{ReqContext: ctx, ExternalUser: extUser, SignupAllowed: setting.AuthProxyAutoSignUp}
