...
type DashboardInputMissingError struct{ VariableName string }
...
func ImportDashboard(cmd *ImportDashboardCommand) error {
	...
	evaluator := &DashTemplateEvaluator{template: dashboard.Data, inputs: cmd.Inputs}
	generatedDash, err := evaluator.Eval()
	if err != nil {
		return err
	}
	saveCmd := m.SaveDashboardCommand{Dashboard: generatedDash, OrgId: cmd.OrgId, UserId: cmd.User.UserId, Overwrite: cmd.Overwrite, PluginId: cmd.PluginId, FolderId: cmd.FolderId}
	dto := &dashboards.SaveDashboardDTO{OrgId: cmd.OrgId, Dashboard: saveCmd.GetDashboardModel(), Overwrite: saveCmd.Overwrite, User: cmd.User}
	savedDash, err := dashboards.NewService().ImportDashboard(dto)
	if err != nil {
		return err
	}
	cmd.Result = &PluginDashboardInfoDTO{PluginId: cmd.PluginId, Title: savedDash.Title, Path: cmd.Path, Revision: savedDash.Data.Get("revision").MustInt64(1), FolderId: savedDash.FolderId, ImportedUri: "db/" + savedDash.Slug, ImportedUrl: savedDash.GetUrl(), ImportedRevision: dashboard.Data.Get("revision").MustInt64(1), Imported: true}
	return nil
}
