...
func TestMapTable(t *testing.T) {
	...
	defer _logClusterCodePath()
	dpw := NewDatasourcePluginWrapper(log.New("test-logger"), nil)
	source := &datasource.Table{Columns: []*datasource.TableColumn{{Name: "column1"}, {Name: "column2"}}, Rows: []*datasource.TableRow{{Values: []*datasource.RowValue{{Kind: datasource.RowValue_TYPE_BOOL, BoolValue: true}, {Kind: datasource.RowValue_TYPE_INT64, Int64Value: 42}}}}}
	want := &tsdb.Table{Columns: []tsdb.TableColumn{{Text: "column1"}, {Text: "column2"}}}
