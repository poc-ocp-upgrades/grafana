...
func (tw *DatasourcePluginWrapper) Query(ctx context.Context, ds *models.DataSource, query *tsdb.TsdbQuery) (*tsdb.Response, error) {
	...
	pbQuery := &datasource.DatasourceRequest{Datasource: &datasource.DatasourceInfo{Name: ds.Name, Type: ds.Type, Url: ds.Url, Id: ds.Id, OrgId: ds.OrgId, JsonData: string(jsonData), DecryptedSecureJsonData: ds.SecureJsonData.Decrypt()}, TimeRange: &datasource.TimeRange{FromRaw: query.TimeRange.From, ToRaw: query.TimeRange.To, ToEpochMs: query.TimeRange.GetToAsMsEpoch(), FromEpochMs: query.TimeRange.GetFromAsMsEpoch()}, Queries: []*datasource.Query{}}
	for _, q := range query.Queries {
		modelJson, _ := q.Model.MarshalJSON()
		pbQuery.Queries = append(pbQuery.Queries, &datasource.Query{ModelJson: string(modelJson), IntervalMs: q.IntervalMs, RefId: q.RefId, MaxDataPoints: q.MaxDataPoints})
	}
	pbres, err := tw.DatasourcePlugin.Query(ctx, pbQuery)
	if err != nil {
		return nil, err
	}
	res := &tsdb.Response{Results: map[string]*tsdb.QueryResult{}}
	for _, r := range pbres.Results {
		qr := &tsdb.QueryResult{RefId: r.RefId, Series: []*tsdb.TimeSeries{}, Tables: []*tsdb.Table{}}
		if r.Error != "" {
		...
		for _, s := range r.GetSeries() {
			...
			qr.Series = append(qr.Series, &tsdb.TimeSeries{Name: s.Name, Tags: s.Tags, Points: points})
		}
...
func (tw *DatasourcePluginWrapper) mapTable(t *datasource.Table) (*tsdb.Table, error) {
	...
	for _, c := range t.GetColumns() {
		table.Columns = append(table.Columns, tsdb.TableColumn{Text: c.Name})
	}
