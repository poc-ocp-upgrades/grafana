...
func autoUpdateAppDashboard(pluginDashInfo *PluginDashboardInfoDTO, orgId int64) error {
	...
	updateCmd := ImportDashboardCommand{OrgId: orgId, PluginId: pluginDashInfo.PluginId, Overwrite: true, Dashboard: dash.Data, User: &m.SignedInUser{UserId: 0, OrgRole: m.ROLE_ADMIN}, Path: pluginDashInfo.Path}
...
func syncPluginDashboards(pluginDef *PluginBase, orgId int64) {
	...
	cmd := m.UpdatePluginSettingVersionCmd{OrgId: appSetting.OrgId, PluginId: appSetting.PluginId, PluginVersion: pluginDef.Info.Version}
