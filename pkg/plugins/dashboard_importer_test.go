...
func TestDashboardImport(t *testing.T) {
	...
	pluginScenario("When importing a plugin dashboard", t, func() {
		...
		cmd := ImportDashboardCommand{PluginId: "test-app", Path: "dashboards/connections.json", OrgId: 1, User: &m.SignedInUser{UserId: 1, OrgRole: m.ROLE_ADMIN}, Inputs: []ImportDashboardInput{{Name: "*", Type: "datasource", Value: "graphite"}}}
	...
	Convey("When evaling dashboard template", t, func() {
		...
		evaluator := &DashTemplateEvaluator{template: template, inputs: []ImportDashboardInput{{Name: "*", Type: "datasource", Value: "my-server"}}}
