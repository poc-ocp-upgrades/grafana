...
func TestMissingPath(t *testing.T) {
	...
	Convey("ls command", t, func() {
		...
		Convey("Missing path", func() {
			commandLine := &commandstest.FakeCommandLine{CliArgs: []string{"ls"}, GlobalFlags: &commandstest.FakeFlagger{Data: map[string]interface{}{"path": ""}}}
		...
		Convey("Path is not a directory", func() {
			commandLine := &commandstest.FakeCommandLine{CliArgs: []string{"ls"}, GlobalFlags: &commandstest.FakeFlagger{Data: map[string]interface{}{"path": "/var/lib/grafana/plugins"}}}
			s.IoHelper = &commandstest.FakeIoUtil{FakeIsDirectory: false}
			Convey("should return error", func() {
				err := lsCommand(commandLine)
				So(err, ShouldNotBeNil)
			})
