...
func (r releaseLocalSources) prepareRelease(baseArchiveUrl, whatsNewUrl string, releaseNotesUrl string, nightly bool) (*release, error) {
	...
	rel := release{Version: buildData.version, ReleaseDate: time.Now().UTC(), Stable: false, Beta: false, Nightly: nightly, WhatsNewUrl: whatsNewUrl, ReleaseNotesUrl: releaseNotesUrl, Builds: buildData.builds}
	return &rel, nil
}
	...
	return func(path string, f os.FileInfo, err error) error {
		...
		for _, archive := range archiveTypes {
			if strings.HasSuffix(f.Name(), archive.urlPostfix) {
				...
				data.builds = append(data.builds, build{Os: archive.os, Url: archive.getUrl(baseArchiveUrl, version, NIGHTLY), Sha256: string(shaBytes), Arch: archive.arch})
				return nil
			}
		}
		return nil
