...
func main() {
	...
	if enterprise {
		...
		var err error
		buildArtifacts, err = filterBuildArtifacts([]artifactFilter{{os: "deb", arch: "amd64"}, {os: "rhel", arch: "amd64"}, {os: "linux", arch: "amd64"}, {os: "win", arch: "amd64"}})
		if err != nil {
	...
	if fromLocal {
		path, _ := os.Getwd()
		builder = releaseLocalSources{path: path, artifactConfigurations: buildArtifacts}
	} else {
		builder = releaseFromExternalContent{getter: getHttpContents{}, rawVersion: version, artifactConfigurations: buildArtifacts}
	}
	p := publisher{apiKey: apiKey, apiUri: "https://grafana.com/api", product: product, dryRun: dryRun, enterprise: enterprise, baseArchiveUrl: baseUrl, builder: builder}
