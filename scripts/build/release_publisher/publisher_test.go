...
func TestPreparingReleaseFromLocal(t *testing.T) {
	...
	builder = releaseLocalSources{path: testDataPath, artifactConfigurations: completeBuildArtifactConfigurations}
	...
	expectedArch := "amd64"
	expectedOs := "win"
	builder = releaseLocalSources{path: testDataPath, artifactConfigurations: []buildArtifact{{os: expectedOs, arch: expectedArch, urlPostfix: ".windows-amd64.zip"}}}
...
func TestFilterBuildArtifacts(t *testing.T) {
	...
	_, err := filterBuildArtifacts([]artifactFilter{{os: "foobar", arch: "amd64"}})
	if err == nil {
